...
work_dir = os.environ.get("WORK_DIR", "work_dirs/glm53_sft")
...
# get_model_config_from_hf dispatches Glm5NextConfig.model_type=="glm5_next" to
# Glm53TextMoEConfig.from_hf, which reads the nested text_config directly -- this is the
# text-only SFT path (design doc F6 point 2); VL training goes through Glm53BaseConfig instead.
model_cfg = get_model_config_from_hf(GLM5_3_MODEL_PATH)
...
model_cfg.attention.sparse_mla_backend = os.environ.get("SPARSE_MLA_BACKEND", "flash_mla_cudnn").strip().lower()
...
cache_tag = os.environ.get("CACHE_TAG", f"glm53_{sample_max_length}")
