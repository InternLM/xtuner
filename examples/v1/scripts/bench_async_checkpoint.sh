...
#
# Runs async first, removes checkpoints, then runs sync,
...
#   SAVE_OPTIM    - "1" (default) save optimizer, "0" skip optimizer
...
export SAVE_OPTIM="${SAVE_OPTIM:-1}"
...
SUFFIX="${MODEL}_${TIMESTAMP}_interval_${CKPT_INTERVAL}"
if [ "$SAVE_OPTIM" = "0" ]; then
    SUFFIX="${SUFFIX}_no_optim"
fi

SYNC_LOG="$LOG_DIR/xtuner_sync_${SUFFIX}.log"
ASYNC_LOG="$LOG_DIR/xtuner_async_${SUFFIX}.log"

# Extract the Trainer's experiment directory (WORK_DIR/<timestamp>) from a log
# file by parsing the DCP save path.
get_exp_dir_from_log() {
    local log_file="$1"
    # Match: DCP save for <path>/checkpoints/... or DCP async_save for <path>/checkpoints/...
    # The experiment dir is everything before "/checkpoints/"
    local exp_dir
    exp_dir=$(grep -oP '(?<=DCP (save|async_save) for )\S+' "$log_file" \
              | head -1 \
              | sed 's|/checkpoints/.*||')
    echo "$exp_dir"
}
...
cleanup_checkpoints() {
    ...
    local exp_dir
    exp_dir=$(get_exp_dir_from_log "$log_file")

    if [ -n "$exp_dir" ] && [ -d "$exp_dir" ]; then
        du -sh "$exp_dir" 2>/dev/null || true
        rm -rf "$exp_dir"
        echo "Removed experiment dir: $exp_dir"
    else
        echo "No experiment dir found from log: $log_file"
    fi

    if [ -f "${WORK_DIR}/.xtuner" ]; then
        rm -f "${WORK_DIR}/.xtuner"
        echo "Removed meta file: ${WORK_DIR}/.xtuner"
    fi

    if [ -f "${WORK_DIR}/meta.json" ]; then
        rm -f "${WORK_DIR}/meta.json"
        echo "Removed legacy meta file: ${WORK_DIR}/meta.json"
    fi
...
echo "Save optimizer: $SAVE_OPTIM"
echo "Work dir:       $WORK_DIR"
echo "Run order:      async -> cleanup -> sync -> cleanup"
...
echo "============================================"
env ASYNC_CKPT=1 SAVE_OPTIM="$SAVE_OPTIM" \
...
cleanup_checkpoints "ASYNC" "$ASYNC_LOG"

# ---- Run 2/2: Sync ----
echo "============================================"
echo " [2/2] Running SYNC mode..."
echo "============================================"
env ASYNC_CKPT=0 SAVE_OPTIM="$SAVE_OPTIM" \
    torchrun --nproc_per_node="$NPROC" xtuner/v1/train/cli/sft.py \
    --config "$CONFIG" \
    2>&1 | tee "$SYNC_LOG"
echo ""
...
cleanup_checkpoints "SYNC" "$SYNC_LOG"
