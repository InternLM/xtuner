...
# 32 GPUs = 4 nodes x 8 GPUs per node.
#
# Qwen3.6 reuses the existing Qwen3.5 XTuner model/tokenize implementation.
# MTP stays disabled in sft_qwen35_35b_256k_32gpu.py, so only the compatible
# Qwen3.6 main model, vision tower, and projector weights are trained.
#
# Muon currently does not support EP > 1 in this XTuner version, so use
# EP=1 and shard the language model over all 32 ranks with FSDP. SP=8 splits
...
# Its Qwen3.5 name is retained intentionally: Qwen3.6 has the same non-MTP
# architecture; CHAT_TEMPLATE_NAME selects the dedicated internal Qwen3.6
# renderer without changing the remaining model or training path.
config_file="${script_dir}/sft_qwen35_35b_256k_32gpu.py"
meta_data_path="${META_DATA_PATH:-${script_dir}/meta.json}"

model_path="${MODEL_PATH:?Set MODEL_PATH to the Qwen3.6 model snapshot directory}"
...
meta_name=${meta_name%.*}
run_tag="${meta_name}-32gpu-ep1-sp8-muon-$(date +%Y%m%d-%H%M%S)"
job_name="qwen36-sft-no-mtp-${run_tag}"
...
rjob submit \
    ...
    -e CHAT_TEMPLATE_NAME=qwen3.6-vl \
    ...
    -- bash -lc '
        ...
        export LOG_FILE="${LOG_DIR}/qwen36-sft-no-mtp-32gpu-ep1-sp8-muon-node${TORCHRUN_NODE_RANK:-unknown}.log"
