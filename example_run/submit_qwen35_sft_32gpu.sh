...
model_path="${MODEL_PATH:?Set MODEL_PATH to the model snapshot directory}"
...
meta_name=${meta_name%.*}
run_tag="${meta_name}-$(date +%Y%m%d-%H%M)"
...
rjob submit \
    ...
    -- bash -lc '
        ...
        export LOG_FILE="${LOG_DIR}/qwen35-sft-node${TORCHRUN_NODE_RANK:-unknown}.log"
