...
class TestMoETrainEngineDeepEPExpertTP(DeterministicDDPTestCase):
    def test_deepep_matches_all2all_with_same_expert_tp_topology(self) -> None:
        ...
        assert (
            engine_all2all.model.expert_tp_mesh.size() == engine_deepep.model.expert_tp_mesh.size() == expert_tp_size
        )
        ...
        assert [len(group["params"]) for group in engine_all2all.optimizer.param_groups] == [
    ...
    def test_deepep_expert_tp_domino_micro_batch_matches_sync_baseline(self) -> None:
        ...
        # Domino 的 integration contract 是与同步 engine 的训练行为一致；virtual-route
        # 扩展与 async output/gradient 由 dispatcher 六阶段 public-API 测试直接覆盖。
        torch.testing.assert_close(
