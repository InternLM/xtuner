...
# 因此本模块执行期间用 Cutlass 后端跑真实 grouped-GEMM。
import parametrize
import torch
    ...
    with torch.no_grad():
        for name, param in engine_tpep.model.named_parameters():
            ...
            if isinstance(module, GroupedLinear) and getattr(module, "tp_enabled", False):
                ...
                ref_full = _full_tensor(getattr(ref_module, param_name).detach()).to(
...
def _slice_tpep_weight(
    grouped_linear: GroupedLinear, full_weight: torch.Tensor, *, fused_gate_up: bool
) -> torch.Tensor:
...
def _slice_tpep_bias(grouped_linear: GroupedLinear, full_bias: torch.Tensor) -> torch.Tensor:
    ...
    bias_shape = (
        grouped_linear.bias.to_local().shape if isinstance(grouped_linear.bias, DTensor) else grouped_linear.bias.shape
    )
...
class TestMoETrainEngineExpertTPOnly(DeterministicDDPTestCase):
    ...
    def test_expert_tp_only_matches_single_with_distinct_source_slices(self, device: str, expert_tp_size: int) -> None:
        pg = self.create_pg(device)
    ...
    def test_expert_tp_only_domino_micro_batch_matches_sync_baseline(self, device: str, expert_tp_size: int) -> None:
        pg = self.create_pg(device)
...
class TestMoETrainEngineTPEP(DeterministicDDPTestCase):
    ...
    def test_tpep_forward_backward_matches_single(self, device: str, ep_size: int, expert_tp_size: int) -> None:
