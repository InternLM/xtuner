...
def test_query_tail_padding_uses_an_empty_causal_range():
    ...
    assert padded_starts.min() == starts.min()
    assert padded_ends.max() == ends.max()


def test_query_chunk_padding_is_a_noop_for_aligned_rows():
    q = torch.randn(4, 2, 4, dtype=torch.bfloat16)
    weights = torch.randn(4, 2)
    starts = torch.tensor([0, 0, 0, 0], dtype=torch.int32)
    ends = torch.tensor([1, 2, 3, 4], dtype=torch.int32)

    outputs = _pad_tilelang_indexer_query_chunk(
        q,
        weights,
        starts,
        ends,
    ...
    assert outputs[-1] == 4
    for actual, expected in zip(outputs[:-1], (q, weights, starts, ends), strict=True):
        torch.testing.assert_close(actual, expected)


def test_query_chunk_padding_rejects_nonpositive_block():
    with pytest.raises(ValueError, match="block_q must be positive"):
        _pad_tilelang_indexer_query_chunk(
            torch.empty(1, 2, 4, dtype=torch.bfloat16),
            torch.empty(1, 2),
...
def test_indexer_forwards_query_chunk_to_selector():
    ...
    def fake_selector(q, k, weights, seq_ctx, *, index_head_dim, index_topk, query_chunk_size=None):
        ...
        calls["shapes"] = (q.shape, k.shape, weights.shape, seq_ctx)
    ...
    assert calls["shapes"][:3] == (torch.Size((1, 5, 2, 4)), torch.Size((1, 5, 4)), torch.Size((1, 5, 2)))
    assert output["dsa_topk_ids"].dtype == torch.int32
    assert output["dsa_topk_ids"].is_contiguous()


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("query_chunk_size", [None, 1, 3, 4, 5, 66, 128])
def test_query_chunk_orchestration_keeps_aligned_launches(monkeypatch, query_chunk_size):
    ...
    k = torch.empty(1, 4096, 128, device="cuda", dtype=torch.bfloat16)
    weights = torch.empty(1, query_len, 32, device="cuda", dtype=torch.float32)
    seq_ctx = SequenceContext.from_input_ids(
        (torch.arange(31, device="cuda").unsqueeze(0), torch.arange(35, device="cuda").unsqueeze(0)),
        device="cuda",
    )
    ...
    for call_idx, (rows, starts, ends) in enumerate(calls):
        ...
        valid_rows = (
            query_len if query_chunk_size is None else min(query_chunk_size, query_len - call_idx * query_chunk_size)
        )
        if rows > valid_rows:
...
def test_query_chunked_selector_matches_one_shot_on_packed_tail():
    ...
    # both the packed boundary and the query chunk tail non-aligned.  The
    # physical K buffer is deliberately larger than the packed query span so
    # this test exercises a real q-tail without relying on the baseline K-tail
    # behavior of clean_logits_.
    lengths = (2047, 2050)
