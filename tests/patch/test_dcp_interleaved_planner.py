...
class TestDCPInterleavedPlanner(DeterministicDDPTestCase):
    ...
    def test_interleaved_round_trip(self, device: str) -> None:
        ...
        optimizer_moment._local_tensor.add_(1000)
        moment_before = optimizer_moment._local_tensor.clone()

        dp_mesh = init_device_mesh("cuda", (self.world_size,), mesh_dim_names=("dp",))
        regular_global = torch.arange(32, device="cuda", dtype=torch.float32).reshape(8, 4)
        regular = distribute_tensor(regular_global, dp_mesh, (Shard(0),))
        regular_before = regular._local_tensor.clone()
        plain = torch.tensor([3.0, 5.0], device="cuda")
        ...
        state = {
            "model": {"w": src, "regular": regular, "plain": plain},
            "optimizer": {
                "state": {"w": {"exp_avg": optimizer_moment}},
                "param_groups": [{"params": ["w"], "lr": 1e-3}],
            },
        }
        dcp.save(state, checkpoint_id=ckpt, planner=InterleavedShardSavePlanner())
        dist.barrier()

        dst = _build_interleaved_dtensor(ep_size=2, tp_size=2)
        dst._local_tensor.zero_()
        dst_moment = _build_interleaved_dtensor(ep_size=2, tp_size=2)
        dst_moment._local_tensor.zero_()
        regular_dst = distribute_tensor(torch.zeros_like(regular_global), dp_mesh, (Shard(0),))
        plain_dst = torch.zeros_like(plain)
        loaded_state = {
            "model": {"w": dst, "regular": regular_dst, "plain": plain_dst},
            "optimizer": {
                "state": {"w": {"exp_avg": dst_moment}},
                "param_groups": [{"params": ["w"], "lr": 0.0}],
            },
        }
        dcp.load(loaded_state, checkpoint_id=ckpt, planner=InterleavedShardLoadPlanner())
        dist.barrier()
        ...
        self.assertTrue(torch.equal(moment_before, dst_moment._local_tensor), "optimizer state mismatch")
        self.assertTrue(torch.equal(regular_before, regular_dst._local_tensor), "regular DTensor mismatch")
        self.assertTrue(torch.equal(plain, plain_dst), "plain tensor mismatch")
        self.assertEqual(loaded_state["optimizer"]["param_groups"][0]["lr"], 1e-3)
