...
from xtuner.v1.module.dispatcher.base import DispacherInterface, NaiveDispatcher
...
class TestNoETorchAll2AllDispatcher(DistributedTestBase):
    ...
    def test_dispatch_and_combine(self, dtype, device):
        ...
        all2all_results = self._dispatcher_call(
            dispatcher=all2all_dispatcher, hidden_states=hidden_states, topk_ids=topk_idx, topk_weights=topk_weights
        )

        self.assertTrue(
            torch.allclose(noep_results["hidden_states"], all2all_results["hidden_states"], atol=1e-6, rtol=1e-4)
        )
