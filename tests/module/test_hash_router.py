...
def _make_logits(num_tokens: int, n_routed_experts: int = 8) -> torch.Tensor:
    return torch.randn(num_tokens, n_routed_experts)
...
class TestHashRouter:
    def test_deterministic_routing(self):
        ...
        logits = _make_logits(input_ids.numel())
        first: RouterResults = router(logits=logits, input_ids=input_ids)
        second: RouterResults = router(logits=logits, input_ids=input_ids)
    ...
    def test_topk_weights_come_from_the_learned_gate(self):
        """Weights are the gate's scores at the hashed indices, renormalised.

        Hash routing freezes *which* experts a token reaches, not how their outputs are
        combined — the V4 reference's ``Gate.forward`` overrides only ``indices`` and falls
        through to the shared ``original_scores.gather(1, indices)`` tail. A router that
        returned uniform ``1 / top_k`` here would drop the gate entirely.
        """
        num_experts_per_tok = 2
        n_routed_experts = 8
        router = _make_router(num_experts_per_tok=num_experts_per_tok, n_routed_experts=n_routed_experts)
        input_ids = torch.tensor([1, 2, 3, 4, 5], dtype=torch.long)
        logits = _make_logits(input_ids.numel(), n_routed_experts)

        results = router(logits=logits, input_ids=input_ids)
        topk_weights = results["topk_weights"]

        scores = logits.sigmoid()
        expected = scores.gather(1, results["topk_ids"])
        expected = expected / expected.sum(dim=-1, keepdim=True)
        assert torch.allclose(topk_weights, expected, atol=1e-6)
        assert torch.allclose(topk_weights.sum(dim=-1), torch.ones(input_ids.shape[0]))
        assert not torch.allclose(topk_weights, torch.full_like(topk_weights, 1.0 / num_experts_per_tok))

    def test_router_scaling_factor_applied(self):
        router = _make_router(router_scaling_factor=2.5)
        input_ids = torch.tensor([1, 2, 3], dtype=torch.long)
        logits = _make_logits(input_ids.numel())

        scaled = router(logits=logits, input_ids=input_ids)["topk_weights"]
        router.router_scaling_factor = 1.0
        unscaled = router(logits=logits, input_ids=input_ids)["topk_weights"]
    ...
    def test_topk_ids_within_range(self):
        ...
        results = router(logits=_make_logits(64, n_routed_experts), input_ids=input_ids)
    ...
    def test_packed_input_ids(self):
        ...
        results = router(logits=_make_logits(total_tokens), input_ids=input_ids)
    ...
    def test_missing_input_ids_raises(self):
        ...
        with pytest.raises(AssertionError, match="HashRouter requires `input_ids`"):
            router(logits=_make_logits(3), input_ids=None)

    def test_missing_logits_raises(self):
        router = _make_router()
        with pytest.raises(AssertionError, match="HashRouter needs the gate logits"):
            router(logits=None, input_ids=torch.tensor([1, 2, 3], dtype=torch.long))
