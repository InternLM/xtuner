...
REPLAY_BUFFER_CONFIGS = [
    pytest.param(SyncReplayBufferConfig, id="sync_fifo"),
    pytest.param(AsyncReplayBufferConfig, id="async_staleness"),
]
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_common_query_count_and_take_batch_contract(replay_buffer_config_cls):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_common_empty_and_zero_size_operations_are_noops(replay_buffer_config_cls):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_common_put_normalizes_generated_rollout_state(replay_buffer_config_cls):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_common_put_expires_stale_group_and_resets_response(replay_buffer_config_cls):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_common_refresh_staleness_contract(replay_buffer_config_cls):
    ...
    async def case():
        ...
        await replay_buffer.put([make_rollout_state(2, status=Status.ABORTED, response_model_steps=[1])], "task")
        ...
        await filtered_buffer.put([make_rollout_state(4, status=Status.ABORTED, response_model_steps=[1])], "task")
...
def test_sync_get_returns_fifo_order():
...
def test_async_get_prefers_higher_staleness():
...
def test_async_get_uses_fifo_as_staleness_tie_breaker():
...
def test_sync_save_resume_preserves_fifo_sampling_order(tmp_path):
...
def test_async_save_resume_preserves_staleness_sampling_order(tmp_path):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_save_resume_preserves_rollout_state_fields(replay_buffer_config_cls, tmp_path):
...
@pytest.mark.parametrize("replay_buffer_config_cls", REPLAY_BUFFER_CONFIGS)
def test_save_resume_preserves_object_refs(replay_buffer_config_cls, tmp_path):
    ...
    try:
        ...
        pytest.skip(f"Ray init failed for replay buffer ObjectRef test: {exc}")
