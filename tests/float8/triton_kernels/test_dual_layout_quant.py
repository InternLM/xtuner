...
def test_per_tile_quant_with_trans_per_tile_matches_reference(
    ...
    _assert_equal(actual[0], row_reference[0], "row fp8")
    _assert_equal(actual[1], row_reference[1], "row scales")
    _assert_equal(actual[2][:, :meaningful_tokens], trans_reference[0][:, :meaningful_tokens], "transposed fp8")
    _assert_equal(
...
def test_grouped_linear_forward_and_backward_match_reference(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _assert_equal(output_actual, output_reference, "grouped linear output")
    _assert_equal(x_actual.grad, dx_reference, "grouped linear input gradient")
    _assert_equal(layer.weight.grad, dw_reference, "grouped linear weight gradient")
