...
def test_agrs_rejects_expert_tp_at_model_build() -> None:
    config = _tiny_moe_cfg()
    config.dispatcher = "agrs"
    config.ep_size = 8
    config.n_routed_experts = 16
    config.num_experts_per_tok = 8
    config.router = GreedyRouterConfig(
    ...
    with pytest.raises(NotImplementedError, match="AGRS with ExpertTP is not supported"):
        config.build()
...
class TestMoEExpertTPWithoutEP(DeterministicDDPTestCase):
    ...
    def test_hsdp_rejects_expert_tp_before_sharding(self) -> None:
        pg = self.create_pg("cuda")
        rank = dist.get_rank()
        torch.cuda.set_device(rank % torch.cuda.device_count())
        ...
        try:
            model = _tiny_moe_cfg().build()
            with self.assertRaisesRegex(NotImplementedError, "HSDP with ExpertTP is not supported"):
                model.fully_shard(FSDPConfig(ep_size=1, hsdp_sharding_size=1))
        finally:
            dist.barrier()
            dist.destroy_process_group(pg)
