...
class TestCheckpointWrapper:
    ...
    def test_frozen_module_with_detached_inputs_does_not_force_replay(self):
        # A frozen vision block has neither grad inputs nor trainable parameters. Its output is a
        # constant for downstream training, so checkpoint must not create an unusable replay edge.
        wrapped = apply_activation_checkpointing(_ParameterOnlyBlock().requires_grad_(False))
        detached_inputs = torch.tensor([3.0])
        downstream_weight = nn.Parameter(torch.tensor([4.0]))

        output = wrapped(detached_inputs)
        (output * downstream_weight).sum().backward()

        assert wrapped.weight.grad is None
        torch.testing.assert_close(downstream_weight.grad, output.detach())
