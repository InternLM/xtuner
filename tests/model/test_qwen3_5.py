            ...
            if dist.get_rank() == 0:
                ...
                for key in origin_index["weight_map"].keys():
                    ...
                    if saved_safetensor_name not in cache_save_fh:
                        cache_save_fh[saved_safetensor_name] = safe_open(saved_sf_fh_name, framework="pt")

                    origin_tensor = cache_save_fh[origin_safetensor_name].get_tensor(key)
                    saved_tensor = cache_save_fh[saved_safetensor_name].get_tensor(key)
