...
class TestReduceSumEndToEnd(_ReduceSumDDPTestCase):
    ...
    def test_display_loss_is_per_rank_no_all_reduce(self):
        ...
        # balancing_loss_global is all-reduced inside the loss context, so unlike the per-rank losses
        # above it MUST be identical on every rank even with distinct per-rank data.
        gvals = [torch.zeros((), device="cuda") for _ in range(world)]
        dist.all_gather(gvals, torch.tensor(info["logs_info"]["reduced_balancing_loss_global"], device="cuda"))
        if rank == 0:
            assert torch.isfinite(torch.stack(gvals)).all(), "non-finite global balancing display loss"
            assert (gvals[0] - gvals[1]).abs() < 1e-5, "global balancing loss differs across ranks (not all_reduced?)"
...
class TestBalancingLossReduceSum(_ReduceSumDDPTestCase):
    ...
    def test_balancing_local_sum_matches_global(self):
        ...
        ctx = cfg.build()
        # Non-padding token count is set once at build time (set_non_pad_token), not threaded per call.
        ctx.set_non_pad_token(n_tokens)
        for layer in range(n_layers):
            ctx.accumulate(layer_idx=layer, router_weights=rw_new[layer])
    ...
    def test_zloss_local_matches_global(self):
        ...
        ctx = cfg.build()
        # Token counts are set once at build time (set_token_counts), not threaded through accumulate().
        ctx.set_token_counts(num_tokens_local, num_tokens_global)
        z_new = torch.zeros((), device=device)
        for layer in range(n_layers):
            z_new = z_new + ctx.accumulate(router_logits=logits_new[layer])
        z_new.backward()
        g_new = w_new.grad.clone()
...
class TestAuxTokenCountsBuild(_ReduceSumDDPTestCase):
    @property
    def world_size(self) -> int:
        return 4

    def test_build_sets_sp_local_and_global_token_counts(self):
        # The aux token counts (z-loss denominators) are computed in build_loss_ctx_batch by
        # sp_split-ing the padding mask and all_reduce-ing the global total, mirroring how CELoss
        # builds its denominators -- so the forward runs NO aux-token collective. Under SP the per-rank
        # count must be this rank's shard, and the global count must be the whole-sequence non-pad
        # total, NOT multiplied by the SP size (the reduce-sum over-count guard). SP mesh == world here
        # (no DP), so the global total equals the full sequence length.
        self.create_pg("cuda")
        device = "cuda"
        world = dist.get_world_size()
        sp_mesh = init_device_mesh("cuda", (world,), mesh_dim_names=("sp",))

        # seq_len not a multiple of world -> padded to a multiple, so the last shard is partial: this
        # exercises the asymmetric-shard arithmetic, not just an even split.
        seq_len = 30
        config = _tiny_moe_config(ep_size=1, balancing=True, z_loss=True)
        model = MoE(config=config).to(device)

        torch.manual_seed(0)
        # Draw ids in [1, 512): pad_token_id is 0, so every position is a real (non-pad) token and the
        # full non-pad count is exactly seq_len.
        ids = torch.randint(1, 512, (1, seq_len + 1), device=device)
        seq_ctx = SequenceContext.from_input_ids(input_ids=(ids[:, :-1],), device=device)
        full_non_pad = int(seq_ctx.mask.sum())
        assert full_non_pad == seq_len

        lcs = model.build_loss_ctx_batch([{"seq_ctx": seq_ctx, "shifted_labels": ids[:, 1:]}], sp_mesh=sp_mesh)
        aux_ctx = lcs[0]["aux"]
        z_ctx = aux_ctx.z_ctx
        balancing_ctx = aux_ctx.balancing_ctx

        local = z_ctx._num_tokens_local
        # This rank owns only its shard, so its count is strictly smaller than the whole sequence.
        assert local < full_non_pad, f"SP-local count {local} should be a shard, not the full {full_non_pad}"

        # The SP shards tile the whole sequence: summing local counts over the SP group recovers it.
        local_sum = torch.tensor(local, device=device)
        dist.all_reduce(local_sum, group=sp_mesh.get_group())
        assert int(local_sum) == full_non_pad, f"SP-local counts must sum to {full_non_pad}, got {int(local_sum)}"

        # The build-side global all_reduce must equal the whole-sequence total, not full_non_pad * world
        # (which is what feeding the un-split full count into the all_reduce would produce).
        assert int(z_ctx._num_tokens_global) == full_non_pad, (
            f"z-loss global token count must be {full_non_pad}, got {int(z_ctx._num_tokens_global)} "
            f"(x{world} would mean the SP shards were double-counted)"
        )
        # Balancing shares the same SP-local count.
        assert balancing_ctx._non_pad_token == local
...
class TestDisplayCalibrateCoefficient:
    """Single-process checks for the CE display coefficient (no dist / GPU).

    ``build_batches`` divides the backward loss_weight by the GLOBAL denominator, so ``calibrate()``
    multiplies back by ``global_denominator / rank_denominator`` to recover this rank's own mean. In a
    single process ``global_denominator == rank_denominator``, so the correct coefficient is exactly
    1.0 for every reduction. The earlier code divided by the raw grad-token count instead of the
    per-rank weight sum, which is only equal to the weight sum for ``token`` reduction -- for
    ``sample`` / ``square`` it produced a wrong (non-unit) coefficient here.
    """

    def _build_coeff(self, loss_reduction: str) -> float:
        from xtuner.v1.loss.ce_loss import CELossConfig, CELossContext, CELossKwargs

        cfg = CELossConfig(loss_reduction=loss_reduction)
        # One sequence, two samples of unequal length (2 and 4 grad tokens) so sample / square weights
        # differ from a uniform per-token weight. All labels valid (none == ignore_idx).
        shifted_labels = torch.arange(6, dtype=torch.long).view(1, 6)
        ctx = CELossContext(cfg, CELossKwargs(shifted_labels=shifted_labels))
        cu_seq_lens = torch.tensor([0, 2, 6], dtype=torch.int32)
        CELossContext.build_batches([ctx], [cu_seq_lens], None)
        return ctx._display_coeff.item()

    def test_token_reduction_coefficient_is_unit(self):
        assert abs(self._build_coeff("token") - 1.0) < 1e-6

    def test_sample_reduction_coefficient_is_unit(self):
        # Old formula (global / grad_token_count) gave 2/6 here; the fix (global / weight_sum) gives 1.
        assert abs(self._build_coeff("sample") - 1.0) < 1e-6

    def test_square_reduction_coefficient_is_unit(self):
        assert abs(self._build_coeff("square") - 1.0) < 1e-6


class TestAuxLossContextCat:
    """Single-process checks that aux contexts collapse to one whole-batch
    context (no dist / GPU).

    Under intra_layer_micro_batch the per-micro-batch aux contexts are merged via ``cat`` and fed the
    concatenated router statistics once, replacing the earlier fan-out to N identical contexts whose
    losses were summed and divided by N. The finalize result must therefore be independent of the
    number of merged chunks -- a guard against reintroducing a per-chunk ``/ batch_size`` factor.
    """

    def _finalize_one(self, n_chunks: int) -> tuple[float, float]:
        from xtuner.v1.loss.moe_loss import (
            BalancingLossConfig,
        ...
        torch.manual_seed(0)
        n_experts, top_k, non_pad = 4, 2, 3
        router_weights = torch.rand(non_pad, n_experts)
        router_logits = torch.randn(non_pad, n_experts)
        tokens_per_expert = torch.tensor([[2, 1, 2, 1]], dtype=torch.long)

        bal = BalancingLossContext.cat([BalancingLossConfig().build() for _ in range(n_chunks)])
        bal.set_non_pad_token(non_pad)
        bal.accumulate(layer_idx=0, router_weights=router_weights)
        bal_loss = bal.finalize(
        ...
        z = ZLossContext.cat([ZLossConfig().build() for _ in range(n_chunks)])
        z.set_token_counts(non_pad, None)
        z.accumulate(router_logits=router_logits)
        z_loss = z.finalize()
        return bal_loss.item(), z_loss.item()

    def test_cat_result_independent_of_chunk_count(self):
        one = self._finalize_one(1)
        many = self._finalize_one(3)
        assert abs(one[0] - many[0]) < 1e-6, "balancing loss depends on merged chunk count"
        assert abs(one[1] - many[1]) < 1e-6, "z-loss depends on merged chunk count"
