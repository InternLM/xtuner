    ...
    with profile_time_and_memory('[Dataset & Dataloader]'):
        ...
        if args.pretrain_datasets:
            ...
            if rank == 0:
                ...
                decoded = tokenizer.decode(msg_collator(_first_batch)['input_ids'][0])
                logger.debug(
                    f'[Pretrain Dataloader] Training Batch(Decode):\n{decoded}')
    ...
    for step in range(start_step, total_steps):
        ...
        for infer_packed_seq in infer_dataloader:
            ...
            if sp_size > 1:
                ...
                infer_input_ids = pad_for_sequence_parallel(
                    infer_input_ids, 0, dim=1)
                ...
                infer_labels = pad_for_sequence_parallel(
                    infer_labels, -100, dim=1)
            ...
            if sp_size > 1:
                ...
                _sp_infer_input_ids = dist.nn.functional.all_gather(infer_input_ids, sp_group)

                infer_input_ids = torch.cat(_sp_infer_input_ids, dim=1)
        ...
        for packed_policy in ppo_loader:
            ...
            with packed_sequence(ppo_num_tokens, sp_size=sp_size):
                ...
                with nullcontext() if update_actor else torch.no_grad():
                    packed_actor_logits = actor_model(ppo_input_ids, use_cache=False).logits
                with torch.no_grad():
                    packed_ref_logits = ref_model(ppo_input_ids, use_cache=False).logits
            ...
            packed_logprobs = gather_logprobs(packed_actor_logits, ppo_labels.clip(0))
            packed_ref_logprobs = gather_logprobs(packed_ref_logits, ppo_labels.clip(0))
            ...
            if sp_size > 1:
                ...
                _sp_packed_ref_logprobs = dist.nn.functional.all_gather(packed_ref_logprobs, sp_group)
            ...
            logprobs = unpack_sequence(packed_logprobs, ppo_num_tokens, dim=1)
            ref_logprobs = unpack_sequence(packed_ref_logprobs, ppo_num_tokens, dim=1)
