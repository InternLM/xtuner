...
from xtuner._lite.accelerate import profile_time_and_memory, varlen_attn_is_available
...
from xtuner._lite.parallel import LengthGroupedSampler, ParallelSampler, setup_parallel
...
def send_to_feishu(web_hook, msg):
    ...
    body = {"msg_type": "text", "content": {"text": f'<at user_id="all">所有人</at>{msg}'}}
...
def sft(args):
    ...
    if args.feishu_webhook and rank == 0:
        ...
        def log_handler(record):
            if record["level"].name == "WARNING":
                send_to_feishu(
                    args.feishu_webhook,
                    f"[WARNING] {record['message']}\n{args.work_dir}",
                )
            elif record["level"].name == "TRACE":
                send_to_feishu(
                    args.feishu_webhook, f"[TRACE] {record['message']}\n{args.work_dir}"
                )
    ...
    fsdp_config = FSDPConfig(
        ...
        max_length=args.max_length * args.mirco_batch_size,
    )
    ...
    for dset_format in args.dset_formats:
        ...
        # and `num_tokens`.
        tokenize_fn = SftTokenizeFunction(tokenizer, chat_template, dset_format)
    ...
    if args.dset_pack_level == "soft":
        train_dataset = SoftPackDataset(
            _datasets, target=args.max_length, blend=args.global_pack
        )
    ...
    collator = SftCollator(
        pack_batch=varlen_attn_is_available(), max_length=args.max_length
    )

    if args.group_by_length:
        sampler = LengthGroupedSampler(
            train_dataset, patched_llm.data_parallel_mesh, args.global_batch_size
        )
    else:
    ...
    if args.resume:
        ...
        if latest_checkpoint:
            ...
            logger.warning(
                f"There is no checkpoint available for resuming training in {args.work_dir}."
            )
    ...
    for step in range(start_step, total_steps):
        ...
        dist.all_reduce(
            rank_grad_tokens, group=patched_llm.data_parallel_mesh.get_group()
        )
        ...
        if grad_norm.isnan() or grad_norm.isinf():
            ...
            logger.warning(
                f"[Step {step}] The grad norm is NaN or Inf, skip this step. Skipped {train_state.if_nan_skip_steps} "
                "steps in total."
            )
            optimizer.zero_grad()
        else:
        ...
        if is_interval(step, total_steps, checkpoint_interval):
            ...
            if args.checkpoint_drop_optimizer:
                ...
                with profile_time_and_memory("[PT Checkpoint]"):
                    ...
                    ) = get_state_dict(llm, optimizer, options=_options)
                    ...
                    ckpt_handle = dcp.async_save(
                        state_dict, checkpoint_id=ckpt_dir, process_group=gloo_group
                    )
