...
from ._pack import _PackDataset
...
class BaseTrainDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        chat_template (ChatTemplate):
            Different models correspond to different chat template.
    ...
    def load_dataset(
        ...
        if self.is_cached(cache_dir):
            logger.info(f'{cache_dir} is a cached dataset that will be loaded '
            ...
            if isinstance(data_files, list):
                ...
                if not os.path.exists(data_files):
                    raise FileNotFoundError(
                        f'{data_files} does not exist, please '
                        'check `data_files`.')
    ...
    def _load_json_dataset(self,
        ...
        for ratio, file in zip(ratios, files):
            ...
            logger.info(f'Loaded json data from {file}, '
    ...
    def _pack_dataset(self, dataset) -> _PackDataset:
        ...
        logger.info('Before pack multi samples to max length: '
                    f'{unpacked_samples} samples')
        logger.info('After pack multi samples to max length: '
    ...
    def filter_non_labels_data(self, dataset: List[dict]) -> List[dict]:
        ...
        logger.info(f'Before filter: {ori_samples} samples')
        logger.info(f'After filter: {new_samples} samples')
        logger.info(f'Filtered {ori_samples - new_samples} samples '
    ...
    def analysis_tokens_labels(self, dataset: List[dict]) -> List[str]:
        ...
        logger.info(f'There are a total of {num_tokens} tokens, '
