...
def trans_per_tile_quant_expand_128x_kernel(
    ...
    for i in tl.range(0, tl.cdiv(group_size, BLOCK_M)):
        ...
        input_quant = input_block / output_block_scale
        output_offset = (
            output_ptr
...
@triton_op("myfp8::trans_per_tile_quant_expand_128x", mutates_args={})
def trans_per_tile_quant_expand_128x(
    ...
    output_scales = input_tensor.new_empty(
        (N, (M + 128 * num_groups - M % 128) // 128), dtype=torch.float32
    )
...
def per_tile_trans_quant_per_expert(x, eps=1e-12, quant_dtype=torch.float8_e4m3fn):
    ...
    scale = scale.float()
    x_fp8 = to_fp8_saturated(x_expand.float() / scale, quant_dtype).view(
        dim, seq_expand
    )
...
if __name__ == "__main__":
    ...
    ne = 32
    x = torch.randn(
        8192 * 8, din, dtype=torch.bfloat16, device="cuda", requires_grad=True
    )
    ...
    if ne == 8:
        ...
        tokens_per_expert = (
            torch.tensor([0, 2 * 2048, 128 - bias, 2 * 2048 - 128 + bias] * 8)
            .cuda()
            .long()
        )
    else:
