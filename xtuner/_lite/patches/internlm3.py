...
from xtuner._lite.modelings.internlm3.modeling_internlm3 import InternLM3RotaryEmbedding, InternLM3ForCausalLM, InternLM3DecoderLayer, InternLM3Attention
...
class CUDAPatchedInternLM3ForCausalLM(CUDAPatchedLlamaForCausalLM):
    ...
    def __init__(self, model, fsdp_config = None):
        ...
        if fsdp_config.max_length is not None:
            ...
            self.patched_model.config.max_position_embeddings = max(fsdp_config.max_length, ori_max_len)
            self.patched_model.model.rotary_emb = InternLM3RotaryEmbedding(self.patched_model.config).to(self.device_type)
