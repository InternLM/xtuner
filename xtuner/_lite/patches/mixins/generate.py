...
class GenerateMixin:
    ...
    @torch.no_grad()
    def build_kv_cache(self, max_batch_size, max_length, block_size = 256, dtype=torch.bfloat16, device='cuda'):
        ...
        for _ in range(self.model_config.num_hidden_layers):
            cache_k = torch.zeros(num_blocks, block_size, num_heads, head_dim, dtype=dtype, device=device)
            cache_v = torch.zeros(num_blocks, block_size, num_heads, head_dim, dtype=dtype, device=device)
    ...
    def generate(self, 
        ...
        for start in range(0, max_batch_size, max_prefill_batch):
            ...
            _packed_ids, _num_tokens = pack_sequence(input_ids[start:start+max_prefill_batch])
            _position_ids = [
                torch.arange(seq.numel()) for seq in input_ids[start:start+max_prefill_batch]
            ]
            ...
            next_block_table.append(block_table[start:start+max_prefill_batch].to(self.device_type).to(torch.int32))
        ...
        while True:
            all_rank_stopped = torch.IntTensor([len(stopped) >= num_sessions]).to(self.device_type)
            torch.distributed.all_reduce(all_rank_stopped, torch.distributed.ReduceOp.MIN)
            if all_rank_stopped:
                break
            ...
            if next_is_prefilling:
                if isinstance(next_input_ids, list):
                    ...
                    seq_lens_q = []
                    seq_lens_k = []
                    for (chunk_input_ids, chunk_pos_ids, 
                         chunk_cu_seq_lens_q, chunk_max_length_q,
                         chunk_cu_seq_lens_k, chunk_max_length_k,
                         chunk_block_table) in zip(
                             next_input_ids, next_position_ids, next_cu_seq_lens_q, next_max_length_q,
                               next_cu_seq_lens_k, next_max_length_k, next_block_table, strict=True):
                        ...
                        chunk_seq_lens_q = chunk_cu_seq_lens_q[1:] -  chunk_cu_seq_lens_q[:-1]
                        chunk_seq_lens_k = chunk_cu_seq_lens_k[1:] -  chunk_cu_seq_lens_k[:-1]
            ...
            _next_seq_lens_q = torch.IntTensor([0] + _next_seq_lens_q).to(self.device_type)
            _next_seq_lens_k = torch.IntTensor([0] + _next_seq_lens_k).to(self.device_type)
