...
def dpo_dataset():
    ...
    tokenizer = AutoTokenizer.from_pretrained("/public/home/lvshuhang/model_space/workspace/internlm_internlm2-chat-1_8b", trust_remote_code=True)
...
def test_dpo_dataset_getitem(dpo_dataset):
    ...
    print(f"{sample}")
    # 检查返回字典的键
    assert all(key in sample for key in ["input_chosen_ids", "chosen_labels", "input_reject_ids", "reject_labels"])
