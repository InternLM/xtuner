...
class ReplayBufferStorage:
    ...
    def __init__(self, replay_buffer_cfg):
        ...
        self.partial_rollout = replay_buffer_cfg.partial_rollout
        ...
        num_buckets = self.tail_batch_candidate_steps + 1

        self._completed_actions: List[List[int]] = [[] for _ in range(num_buckets)]
        self._aborted_actions: List[List[int]] = [[] for _ in range(num_buckets)]
    ...
    def aborted_samples_count(self):
        return sum(len(bucket) for bucket in self._aborted_actions)
    ...
    @property
    def dataset_prompt_count(self) -> int:
    ...
    def _sample_from_aborted_storage(self) -> List[RLDataFlowItem]:
        ...
        self.logger.info(
            f"Sampling aborted action_id: {action_id} from replay buffer, remain aborted samples: {self.aborted_samples_count}"
        )
        return group_samples
    ...
    def _check_completed_samples_expired(self):
        ...
        for version_idx, bucket in enumerate(self._completed_actions):
            if self.tail_batch_candidate_steps > 0 and version_idx >= self.tail_batch_candidate_steps:
                if bucket:
                    self._expired_actions.extend(bucket)
                    self._completed_actions[version_idx] = []
                    self.logger.info(
                        f"Moved {len(bucket)} completed samples with version {version_idx} to expired samples due to exceeding tail_batch_candidate_steps."
                    )
    ...
    def _clear_meta_for_actions(self, replay_meta: ReplayMeta):
        ...
        for observation_id in replay_meta.observation_ids:
            obs_ref = self._observations.pop(observation_id, None)
            ray._private.internal_api.free(obs_ref)
    ...
    def _check_rollout_state_and_insert(self, replay_meta: ReplayMeta):
        ...
        if state == RolloutState.ABORTED:
            if self.tail_batch_candidate_steps == 0:
                ...
                self._aborted_actions[0].append(action_id)
                self.logger.debug(
...
class ReplayBuffer:
    ...
    def get_prerun_state(self, target_batch_size: int):
        self.sample_from_expired_states = self.storage.expired_samples_count >= (
            target_batch_size - self.storage.completed_samples_count
        )
    ...
    def status(self):
        return {
            "completed_samples_count": self.storage.completed_samples_count,
            "aborted_samples_count": self.storage.aborted_samples_count,
            "expired_samples_count": self.storage.expired_samples_count,
            "dataset_prompt_count": self.storage.dataset_prompt_count,
            "prompt_count": self.storage.prompt_count,
        }
