...
class Sampler:
    ...
    def sample_from_datasets(self, env: str, repeat_prompt_k: int) -> List[RLDataFlowItem]:
        ...
        root_id = uuid4().int
        action_id = uuid4().int
        group_data_item: List[RLDataFlowItem] = [RLDataFlowItem()] * repeat_prompt_k
        try:
    ...
    def sample_from_unfinished_buffer(self):  # -> List[RLDataFlowItem]:
        ...
        # group_replay_meta = [self.storage._actions[action_id] for action_id in self.storage._group2actions[prompt_id]]
        # group_samples = []
...
class ReplayBufferStorage:
    ...
    def __init__(self):
        ...
        self._paused: List[int] = []  # List of paused group_ids, 
        self._returned: List[int] = []  # List of returned group_ids, 
        self._actions: Dict[int, ReplayMeta] = {}  # action_id: ReplayMeta
        self._root2actions: Dict[int, List[int]] = defaultdict(list)  # root_id: [action_id, action_id, ...], designed for grpo
        ...
        self._action2observations: Dict[int, List[int]] = defaultdict(list)  # action_id: [observation_id, observation_id, ...]
