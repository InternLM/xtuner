...
class LMDeployWorker(RolloutWorker):
    ...
    async def _handle_routed_experts_response(self, *args, **kwargs):
        # Delegate to the base class: controller-side slicing + RoutedExpertStore
        # handles the cumulative-length bookkeeping that used to live here.
        return await super()._handle_routed_experts_response(*args, **kwargs)

    def _decode_routed_experts(self, routed_experts: Any):
        # lmdeploy pytorch backend may return either:
        #   - ray.ObjectRef (when enable_return_routed_experts=True is passed
        #     through the engine config — default for this backend)
        #   - np.ndarray directly (rarer path)
        # Base-class handler awaits the ref when present; we just pass it
