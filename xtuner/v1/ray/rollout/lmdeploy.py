...
class LMDeployWorker(RolloutWorker):
    ...
    async def _handle_routed_experts_response(self, *args, **kwargs):
        # No backend-specific override: base ``RolloutWorker`` handles ndarray
        # standardization via ``_decode_routed_experts``.  Kept as a thin
        # forwarder so subclassing remains explicit.
        return await super()._handle_routed_experts_response(*args, **kwargs)

    def _decode_routed_experts(self, routed_experts: Any) -> np.ndarray:
        """Standardize lmdeploy's raw ``routed_experts`` payload to ndarray.

        lmdeploy's pytorch backend returns ``routed_experts`` as either an
        ``np.ndarray`` directly or a ``torch.Tensor``-compatible buffer,
        depending on engine version.  We normalise to ``np.ndarray`` so all
        downstream code (worker concat, HTTP wire codec, training worker)
