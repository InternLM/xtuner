    ...
    async def run_single(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "[%s] infer: done rc=%d (%.1fs)", tid,
                infer_result.return_code, time.monotonic() - t1,
            )
            ...
            logger.info(
                "[%s] validate: done total=%.4f (%.1fs)", tid,
                aggregated.total, time.monotonic() - t2,
            )
...
async def _acquire_ready_sandbox(provider: Any, spec: Any) -> tuple[Any, str]:
    ...
    for attempt in range(1, _ACQUIRE_MAX_ATTEMPTS + 1):
        ...
        logger.warning(
            "sandbox %s never became healthy; deleting and retrying", env_id,
        )
        try:
    ...
    raise RuntimeError(
        f"could not acquire a healthy sandbox after {_ACQUIRE_MAX_ATTEMPTS} attempts: {last_err}"
    )


async def _wait_healthy(client: Any) -> bool:
    """Poll /health until it returns ``ok: True`` or the budget is exhausted."""
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    jobs: list[tuple[Path, dict[str, int]]] = [
        (td, {"root_id": r, "action_id": 0, "observation_id": 0})
    ...
    print(
        f"TotalTask: {total} (base={len(base_dirs)} × repeat={repeats}, "
        ...
        async with sem:
            ...
            async with completed_lock:
                ...
                logger.info(
                    ...
                    completed, total, tid, state, took,
                    int(elapsed), int(remaining), rate,
                )
            return result

    results = await asyncio.gather(*[
        _guarded(i, td, uid) for i, (td, uid) in enumerate(jobs)
    ])
    if args.verbose:
        for r in results:
...
_CATEGORIES: list[tuple[str, str]] = [
    ...
    ("upstream_data_missing", r"(?:cp: cannot stat|cannot open|No such file or directory) ['\"]?([^'\"\n]+environment/data[^'\"\n]+)"),
    # Missing deps
...
def _categorize_failure(reason: str) -> tuple[str, str | None]:
    """Return (category, detail).  ``detail`` carries the captured group for
...
def _write_run_report(results: list[dict], out_dir: Path) -> Path:
    ...
    missing_pkgs = sorted({
        s["detail"] for s in failed
        if s.get("category") == "missing_python_pkg" and s.get("detail")
    })
    missing_cmds = sorted({
        s["detail"] for s in failed
        if s.get("category") == "missing_system_cmd" and s.get("detail")
    })
    script_bugs = sorted({
        s["task_id"] for s in failed
        if s.get("category", "").startswith("script_")
    })
...
def _install_async_logging(level: str) -> Any:
    """Route all log records through a queue to a background writer thread.

    At high concurrency, ``logger.info()`` calls otherwise contend on a
    global handler lock while also holding the stderr write — blocking
    whichever asyncio-to-thread worker happens to hold the GIL.  Observed
    symptom: batch runs with verbose logging cascade into gateway
    timeouts / HTTP 404s that don't reproduce with logging off.

    ``QueueHandler`` makes each log call a near-lock-free ``queue.put``;
    a single ``QueueListener`` background thread drains the queue and
    owns the (slow) stderr write.  No contention from the hot path.
    ...
    listener = QueueListener(log_queue, stderr_handler, respect_handler_level=True)
    listener.start()
    return listener
...
def main() -> int:
    ...
    parser.add_argument(
        "--concurrency", type=int, default=1024,
    ...
    parser.add_argument(
        ...
        help="Root log level (DEBUG / INFO / WARNING).",
    )
    args = parser.parse_args()
    if args.lagent_src == "":
        args.lagent_src = None

    listener = _install_async_logging(args.log_level)
    try:
