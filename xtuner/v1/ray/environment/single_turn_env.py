...
class SingleTurnEnvironment(BaseEnvironment):
    ...
    async def generate(self, group_data_items: List[RLDataFlowItem], sample_params: None) -> List[RLDataFlowItem]:
        ...
        if self.rollout_controller:
            ...
            logger.info(f"rollout_responses: {rollout_responses}")
            group_data_items = update_dataflow_item(
                group_data_items, "env.rollout", rollout_responses
            )  # 支持链式更新：env.rollout / env.judger
            logger.info(f"rollout : {group_data_items}")
    ...
    async def run(self, group_data_items: List[RLDataFlowItem], sample_params: None) -> List[RLDataFlowItem]:
        ...
        logger.info(f"after generate: {group_data_items}")
