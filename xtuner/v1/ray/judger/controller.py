...
class JudgerController:
    ...
    async def _call_custom_reward_judger(
        ...
        if self.judger_config.enable_batch_reward:
            ...
            coroutines = [item for item in flat_tasks_with_names]
        ...
        self.logger.info(f"flat_results: {flat_results}")
        # 将不同Judger的RLJudgerResponseItem进行组装
        uid_list = [item.uid.action_id for item in group_data_item]
        judger_response_items_dict = {uid: RLJudgerResponseItem(uid=uid) for uid in uid_list}
        # self.logger.info(f"judger uid_list: {uid_list}")
        ...
        # self.logger.info(f"judger_response_items_dict: {judger_response_items_dict}")
        return list(judger_response_items_dict.values())

    async def run(self, group_data_item: RLDataFlowItem | List[RLDataFlowItem]) -> RLJudgerResponseItem | List[RLJudgerResponseItem]:
        ...
        assert active_reward_judger, f"No active reward judger in {judger_names} found for the given data source {data_source}."
