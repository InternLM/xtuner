...
def _pack_score_idx(score, idx, T_PADDED: tl.constexpr):
    """Pack ``(fp32 score, int32 idx)`` into ``uint64`` for sort-descending
    top-K.
...
def _unpack_idx(packed, T_PADDED: tl.constexpr):
    """Inverse of :func:`_pack_score_idx`: recover the original (signed)
    index."""
...
def indexer_topk_triton(
    ...
    if weights.size(1) != total_q or weights.size(2) != n_heads:
        raise ValueError(f"weights shape {tuple(weights.shape)} incompatible with q {tuple(q.shape)}")
