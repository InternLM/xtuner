...
if _HAS_TRITON:
    ...
    def _rope_split_setup_ctx(ctx: torch.autograd.function.FunctionCtx, inputs: tuple, output: Tensor) -> None:
        x, cos, sin, rope_head_dim, forward = inputs
        ctx.save_for_backward(cos, sin)
        ctx.rope_head_dim = rope_head_dim
        ctx.forward = forward

    def _rope_split_backward(ctx: torch.autograd.function.FunctionCtx, grad_out: Tensor) -> tuple:
