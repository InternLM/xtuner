...
class DeepSeekSparseAttention(nn.Module):
    ...
    * **Attention sink**: a learnable per-head fp32 logit concatenated to the
    ...
    # Only registered on the HCA padded path; declared here so the buffer keeps
    # its Tensor type instead of nn.Module's `Tensor | Module` attribute fallback.
    _compress_topk_pad: torch.Tensor
