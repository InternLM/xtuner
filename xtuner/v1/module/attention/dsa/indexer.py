...
class Indexer(nn.Module):
    ...
    def forward(
        ...
        #
        # The rotation is orthogonal, so applying it to *both* operands leaves the score's
        # inner product unchanged in exact arithmetic — it exists only to spread activation
        # magnitude for the reference's FP4 quantization. HF omits it on both sides, so
        # under HF parity we do too: keeping it would round differently and reorder the
        # top-k, while dropping it changes nothing mathematically. The two calls are
        # deliberately gated by the same condition — rotating one side alone is the
        # mismatched-bases bug this coupling exists to prevent.
        rotate = not hf_parity_enabled()
        if rotate:
        ...
        kv_compressed, compressed_cu_seq_lens = self.compressor(
            hidden_states,
            cu_seq_lens,
        ...
        # The score is an inner product, so both operands must live in the same basis.
        # The V4 reference builds the Indexer's compressor with ``rotate=True`` and
        # rotates its output exactly here (`Compressor.forward`, guarded by `self.rotate`),
        # alongside the `q` rotation above. Rotating only `q` leaves the dot product in
        # mismatched bases and silently reorders the top-k. The outer CSA / HCA
        # compressors are built with ``rotate=False`` and must stay unrotated, which is
        # why this lives in the Indexer rather than in :class:`KVCompressor`.
        if rotate:
            kv_compressed = rotate_activation(kv_compressed)

        # Step 5: gate weights, scaled exactly as V4 reference L418 — the two constants are
        # pre-multiplied into one. Both are global positive scalars, so folding them here
        # cannot change which entries win the top-k; it only moves the rounding.
        #
        # HF instead applies ``softmax_scale`` to the scores and ``n_heads ** -0.5`` to the
        # weights, in fp32. Under HF parity we adopt its association and precision so the
        # selected index sets match bit for bit; see ``_index_scores_hf_parity``.
        ...
        for i in range(num_samples):
            ...
            # Step 6-7: per-head dot, ReLU clipped (V4 reference L420-421), then the
            # head-axis weighted sum -> [1, S_i, T_i].
            if hf_parity_enabled():
                index_score = self._index_scores_hf_parity(q_i, kv_i, w_i)
            else:
    ...
    def _index_scores_hf_parity(self, q: torch.Tensor, kv: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
        # Bit-exact transcription of HF's ``DeepseekV4IndexerScorer.forward``: fp32
        # throughout, ``softmax_scale`` multiplied into the scores rather than folded into
        # the weights, and the same batched-matmul shape — ``[B, S, H, D] @ [B, 1, D, T]``
        # — because a differently shaped contraction over ``D`` rounds differently and the
        # top-k is a discrete function of the result.
        #
        # This is a precision *increase* over the default path, which is deliberately
        # low-precision: the V4 reference feeds the scorer FP4-quantized activations
        # (``fp4_act_quant`` on both ``q`` and the compressed KV) and trains it that way,
        # so bf16 is already far finer than the deployment target. Neither form is more
        # correct — the two constants are global positive scalars and cannot reorder the
        # top-k — so production keeps the reference's cheaper association.
        scores = torch.matmul(q.float(), kv.transpose(-1, -2).float().unsqueeze(1))
...
def _apply_rope(x: torch.Tensor, cos_full: torch.Tensor, sin_full: torch.Tensor) -> torch.Tensor:
    ...
    if cos_full.dim() != 3 or sin_full.dim() != 3:
        raise ValueError(
            f"_apply_rope expects cos/sin of rank 3; got cos {tuple(cos_full.shape)}, sin {tuple(sin_full.shape)}"
        )
    ...
    if x.size(-1) != cos_full.size(-1):
        raise ValueError(f"rope dim mismatch: x last dim {x.size(-1)} != cos last dim {cos_full.size(-1)}")
