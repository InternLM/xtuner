...
def sparse_attn(
    ...
    if hf_parity_enabled():
        return _sparse_attn_hf_parity(q, kv, attn_sink, topk_idxs, softmax_scale, cu_seq_lens)
...
def _sparse_attn_hf_parity(
    ...
    # Bit-exact transcription of HF's ``eager_attention_forward``, selected by
    # ``XTUNER_V4_HF_PARITY``. Three things have to match, and each one was measured to
    # matter on its own:
    #
    #   1. dtype — HF keeps the whole QK / softmax / PV chain in bf16 and subtracts the
    #      row max *in bf16* before the softmax. The default path above runs it in fp32,
    #      which is ~14000x more accurate against an fp64 reference (5e-7 vs 7e-3 relative)
    #      but disagrees with HF in the last bits.
    #   2. reduction width — HF reduces over the *dense* KV axis with the out-of-window
    #      entries masked to ``-inf``; the default path reduces over the gathered top-k.
    #      The masked terms contribute exact zeros, so the two agree mathematically, but
    #      cuBLAS selects its tiling from the contraction length, so a ``k``-wide and a
    #      ``T``-wide reduction pair the surviving terms differently and disagree on a
    #      handful of rows.
    #   3. GEMM shape — HF broadcasts the single KV head with ``repeat_kv`` and issues one
    #      batched ``[B, H, S, D] @ [B, H, D, T]`` matmul. Contracting the same numbers as
    #      a single fused GEMM (e.g. ``einsum("bshd,btd->bsht")``) splits the ``D``
    #      reduction differently and is *not* bitwise, which is why this mirrors the shapes
    #      rather than just the arithmetic.
    #
    # Materialising the dense ``[1, H, S, T]`` logits is why this is test-only: it grows
    # with ``seq_len**2`` where the production path grows with ``seq_len * k``.
    dtype = q.dtype
    total_tokens, num_heads = q.size(1), q.size(2)
    kv_len = kv.size(1)

    valid = topk_idxs >= 0
    valid = valid & _build_horizon_mask(topk_idxs.clamp(min=0), kv_len, cu_seq_lens)
    # Park masked slots on a scratch column so the ``-1`` clamp cannot mark real column 0
    # as attended for a query that never selected it.
    parked = torch.where(valid, topk_idxs.long(), torch.full_like(topk_idxs, kv_len, dtype=torch.long))
    keep = torch.zeros(1, total_tokens, kv_len + 1, dtype=torch.bool, device=q.device)
    keep.scatter_(2, parked, True)
    keep = keep[:, :, :kv_len]

    query = q.transpose(1, 2)  # [1, H, S, D]
    key = kv.unsqueeze(1).expand(-1, num_heads, -1, -1)  # repeat_kv over the single KV head
    attn_weights = torch.matmul(query, key.transpose(2, 3)) * softmax_scale
    attn_weights = attn_weights.masked_fill(~keep.unsqueeze(1), float("-inf"))

    sinks = attn_sink.to(dtype).reshape(1, -1, 1, 1).expand(1, num_heads, total_tokens, 1)
    combined = torch.cat([attn_weights, sinks], dim=-1)
    combined = combined - combined.max(dim=-1, keepdim=True).values
    probs = torch.softmax(combined, dim=-1, dtype=combined.dtype)
    out = torch.matmul(probs[..., :kv_len], key)
