...
def _ensure_cudnn_patches_applied() -> None:
    """One-shot runtime workaround for cudnn 1.24.0 ↔ cu12 cutlass-dsl 4.5.0
    ABI skew.
    ...
    from cudnn.deepseek_sparse_attention.utils.sm90 import (
        primitives as _cudnn_primitives,  # type: ignore[import-not-found]
    )
...
def flash_mla_sparse_attn(
    ...
    """Drop-in replacement for :func:`sparse_attn` using FlashMLA's forward
    kernel.
...
def cudnn_sparse_attn(
    ...
    """Drop-in replacement for :func:`sparse_attn` using FlashMLA fwd + cudnn
    bwd.
