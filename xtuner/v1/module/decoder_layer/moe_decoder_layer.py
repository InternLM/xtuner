...
class MoEActFnConfig(BaseModel):
    ...
    # Whether the shared expert also bounds its SwiGLU pre-activations. DeepSeek-V4's
    # reference and HF both clamp the shared expert exactly like a routed one, so this is
    # the faithful setting — but turning it on changes trained-model numerics, so it stays
    # opt-in rather than silently altering existing runs.
    clamp_shared_expert: bool = False
    ...
    def build(self) -> MoEActFnProtocol:
        ...
        if self.act_type == "clipped_swiglu":
            ...
            # `clamped_swiglu` is a plain SwiGLU, so it has no sigmoid-steepness knob;
            # `clip_alpha` is meaningless here and is deliberately not forwarded.
...
class MoEMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        up = self.up_proj(x)
        # Model families whose routed experts bound the SwiGLU pre-activations (DeepSeek-V4's
        # `swiglu_limit`) apply the same bound to the shared expert. Opt-in: families that do
        # not set a limit keep the unclamped product.
