...
# Single env-switch shared with the rest of the ``mhc`` integration (see
# ``deepseek_v4.py`` / ``hc_block.py``). When set we route the Sinkhorn iteration
# loop through the DeepSeek TileKernels backend (``xtuner.v1.ops.mhc.mhc_sinkhorn``).
...
def hc_split_sinkhorn(
    ...
    # Sinkhorn-Knopp projection: ``softmax`` + ``iters`` alternating row/column
    # normalizations, every step stabilized by ``+ eps``. TileKernels has the
    # whole loop fused into one persistent kernel; the pure-PyTorch path stays
    # bit-equivalent to HF for the default-off case.
    if _USE_MHC_KERNELS and mixes.is_cuda:
        # One TileLang kernel does the scale/base affine + sigmoid (with the
        # ``2 * sigmoid`` post path) + comb-logits split in a single pass.
        from xtuner.v1.ops.mhc import mhc_pre_split_mixes, mhc_sinkhorn

        pre, post, comb = mhc_pre_split_mixes(
            mixes, hc_scale, hc_base, post_mult_value=2.0, pre_eps=eps,
        )
        comb = mhc_sinkhorn(comb, iters, eps)
