    ...
    * :meth:`_shared_experts_forward` — shared expert FFN ± gate (called by post).
        ...
        *,
        ...
        if n_shared_experts > 0:
            self.shared_experts = MoEMLP(
                ...
                mlp_bias=mlp_bias,
                float8_cfg=float8_cfg,
                # Opt-in via `clamp_shared_expert`: the reference and HF both bound the
                # shared expert like a routed one, but enabling it shifts numerics, so the
                # default leaves existing runs untouched.
                swiglu_limit=moe_act_fn_cfg.clip_limit if moe_act_fn_cfg.clamp_shared_expert else None,
            )
    ...
    def _forward(
        ...
        *,
        ...
        """Sequential single-MB layer pass — HC-wrapped attn then HC-wrapped
        ffn.
    ...
    def _micro_batch_forward(
        ...
        """Domino-EP wave pipeline across micro-batches for one V4 decoder
        layer.
