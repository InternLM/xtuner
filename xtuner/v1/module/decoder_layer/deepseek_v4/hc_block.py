...
# Opt-in switch for the HF-bit-parity ``hc_pre`` / ``hc_post`` paths: the all-fp32 RMS +
# fp32 Linear chain that matches HF's ``DeepseekV4HyperConnection.forward`` bitwise.
# Default off → bf16 Linear with cuBLAS fp32 accumulator (~10-20× faster on H100 at
# K=16384/N=24, ~1.5e-2 max abs drift on the layer output).
#
# The flag itself lives in :mod:`xtuner.v1.utils.hf_parity`, which is the single switch for
# every V4 parity site (HC, attention, Indexer) — see that module for the rationale.

# Opt-in switch for the DeepSeek TileKernels ``mhc`` backend (see
# https://github.com/deepseek-ai/TileKernels). When ``XTUNER_USE_MHC_KERNELS=1`` we route the
# HC kernels (``hc_post`` here; ``_hc_head_reduce_compute`` in ``deepseek_v4.py``) through the
# upstream TileLang implementations. ``XTUNER_V4_HF_PARITY`` still wins when set — the bit-
# exact eager path remains the trust anchor. Without this flag the existing default fused
# Triton kernel (``xtuner.v1.ops.hc_post.hc_post_fused``) keeps serving the bf16 fast path.
_USE_MHC_KERNELS = os.getenv("XTUNER_USE_MHC_KERNELS", "0") == "1"


def _hc_post_mhc(x: Tensor, residual: Tensor, post: Tensor, comb: Tensor) -> Tensor:
    """TileKernels ``mhc.post_kernel`` driver for :func:`hc_post`.

    Delegates to :func:`xtuner.v1.ops.mhc.mhc_post`, which wraps both the forward and
    backward TileLang kernels behind a ``torch.library.custom_op`` so autograd flows
    through correctly (returning grads for ``x / residual / post / comb`` via the
    upstream ``_mhc_post_bwd`` kernel).
    """
    from xtuner.v1.ops.mhc import mhc_post
...
def hc_pre(
    ...
    if hf_parity_enabled():
        ...
        if _HAS_QUACK:
            flat_normed = _quack.rmsnorm(x_flat, weight=None, eps=norm_eps)
        else:
    ...
    # Default eager path multiplies fp32 ``pre`` against bf16 ``x_flat`` using PyTorch's
    # TensorIterator auto-promotion — the bf16 input stays bf16 in memory, the product is
    # fp32 only at the reduce. The TileKernels override packs this into one persistent kernel
    # and accumulates in fp32 internally before storing the bf16 output.
    if _USE_MHC_KERNELS and x_flat.is_cuda and x_flat.dtype == torch.bfloat16:
        from xtuner.v1.ops.mhc import mhc_pre_apply_mix

        return mhc_pre_apply_mix(x_flat.view(shape), pre), post, comb
...
def hc_post(x: Tensor, residual: Tensor, post: Tensor, comb: Tensor) -> Tensor:
    ...
    when HF parity is enabled — the bit-exact HF-parity tests keep the eager
    ...
    #   1. ``XTUNER_V4_HF_PARITY=1`` — bit-exact HF eager path, always wins (trust anchor).
    #   2. ``XTUNER_USE_MHC_KERNELS=1`` — DeepSeek TileKernels mhc backend (Hopper-only TileLang).
    #   3. Default — in-tree fused Triton kernel (``hc_post_fused``).
    #   4. Fallback — eager path (CPU / non-bf16 / Triton-less builds).
    if hf_parity_enabled():
        return _hc_post_eager(x, residual, post, comb)
    if _USE_MHC_KERNELS and residual.is_cuda and residual.dtype == torch.bfloat16:
        return _hc_post_mhc(x, residual, post, comb)
...
def _hc_post_eager(x: Tensor, residual: Tensor, post: Tensor, comb: Tensor) -> Tensor:
    """Eager fp32-accumulate ``hc_post`` (HF-bit-parity path; see
    :func:`hc_post`)."""
