    ...
    def finalize(
        ...
        *,
        ...
        if dist.is_initialized():
            ...
            # at world size 1 (tokens_per_expert_global == tokens_per_expert_local, seqlen_global ==
            # valid_tokens); kept so reference / eval (dist uninitialized) still works.
    ...
    def accumulate(
        ...
        *,
        ...
        if num_tokens_global is not None:
            # Distributed: the injected z-loss stays as this rank's local component (its share of the
            # global z-loss, WITHOUT any `× world_size`). Cross-rank aggregation happens on the
            # gradients via the FSDP SUM reduce-scatter; summing over ranks reproduces the global
            # z-loss. Single-process (num_tokens_global is None) keeps `loss = base`, its W=1 case.
