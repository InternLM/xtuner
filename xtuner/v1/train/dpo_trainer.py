...
class DPOTrainer:
    ...
    def _compute_ref_logprobs(
        ...
        with torch.no_grad():
            # Forward chosen through reference model
            ref_chosen_output = self.ref_engine.forward_only(chosen_seq_ctx)
            ref_chosen_logits = _get_field(ref_chosen_output, "logits")

            # Forward rejected through reference model
            ref_rejected_output = self.ref_engine.forward_only(rejected_seq_ctx)
            ref_rejected_logits = _get_field(ref_rejected_output, "logits")

            # Compute log probs
            # NOTE: _gather_logprobs returns token-level logprobs [B, L]
            ref_chosen_token_logprobs = self._gather_logprobs(ref_chosen_logits, chosen_labels)
            ref_rejected_token_logprobs = self._gather_logprobs(ref_rejected_logits, rejected_labels)

            # Compute masks
            chosen_mask = (chosen_labels != -100)
            rejected_mask = (rejected_labels != -100)

            # Sum local log probs and counts
    ...
    def _train_step(self, batch: List[DPOColateItem]) -> Dict[str, float]:
        ...
        for item in batch:
            ...
            rejected_logits = _get_field(rejected_output, "logits")

            # Concat along seq_len to match DPOLossKwargs.shifted_labels layout
            logits = torch.cat([chosen_logits, rejected_logits], dim=1).float()
            ...
            # Compute loss using DPOLossContext's logprob math (logits already computed)
            from xtuner.v1.rl.utils import gather_logprobs as _gather_logprobs

            all_logprobs = _gather_logprobs(logits, shifted_labels)  # [B, L]
            ...
            for loss_type, weight in zip(self.config.loss_cfg.loss_types, self.config.loss_cfg.loss_weights):
                if loss_type == "sigmoid":
                    ...
                    _l = loss_ctx._sft_loss(  # type: ignore[attr-defined]
                        logits=logits[:, : chosen_mask.shape[1]],
                        labels=shifted_labels[:, : chosen_mask.shape[1]],
                        mask=chosen_mask[:, : chosen_mask.shape[1]],
                        loss_weight=loss_weight[:, : chosen_mask.shape[1]],
                    ) * weight
                    # Aggregate SFT loss across SP ranks (each rank only has partial tokens)
                    if self.sp_mesh.size() > 1:
