...
class TrainEngine:
    ...
    def save_dcp(
        ...
        with profile_time_and_memory(f"[DCP Checkpoint to {model_dir}]"):
            ...
            # InterleavedShard (tpep) DTensors — per-expert column-parallel fused MoE weights —
            # are stored per-run by ``InterleavedShardSavePlanner``; the default planner would
            # silently mis-map them (a single contiguous chunk that ignores the interleaving).
            dcp.save(
                model_state,
        ...
        with profile_time_and_memory(f"[DCP Checkpoint to {optimizer_dir}]"):
            if optimizer_dir is not None:
                ...
                # Optimizer state mirrors the param placement, so its InterleavedShard leaves need
                # the same per-run planner.
                dcp.save(
    ...
    def load_dcp(
        ...
        with profile_time_and_memory(f"[Load DCP Model from {model_dir}]"):
            ...
            # InterleavedShard (tpep) DTensors are read per-run by ``InterleavedShardLoadPlanner``,
            # resharding from the global-coordinate checkpoint into this rank's interleaved runs.
            dcp.load(
