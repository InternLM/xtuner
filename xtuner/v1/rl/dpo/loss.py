...
class DPOLossConfig(BaseLossConfig):
    ...
    @property
    def _loss_kwargs_cls(self) -> type["DPOLossKwargs"]:
        return DPOLossKwargs
    ...
    def build(
        ...
        if "chosen_shifted_labels" not in data or "rejected_shifted_labels" not in data:
            return None

        chosen_shifted_labels = data["chosen_shifted_labels"]
        rejected_shifted_labels = data["rejected_shifted_labels"]
        ref_chosen_logprobs = data.get("ref_chosen_logprobs")
        ref_rejected_logprobs = data.get("ref_rejected_logprobs")
        ...
        if sp_mesh is not None and sp_mesh.size() > 1:
            dpo_input = dpo_input.sp_split(sp_mesh)

        loss_kwargs_list = DPOLossContext.build_batches_loss_kwargs(
            [dpo_input], self, sp_mesh=sp_mesh,
        )
        loss_ctx = self.loss_ctx_cls(self, loss_kwargs_list[0])
        return loss_ctx
