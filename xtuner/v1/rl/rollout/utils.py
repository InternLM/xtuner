    ...
    async def postprocess(
        ...
        *,
        ...
        if history_routed_experts is not None and routed_experts is not None:
            ...
            if history_routed_experts_len != history_routed_experts_expect_len:
                self.logger.error(
                    "History routed_experts len mismatch before partial rollout concatenation: "
                ...
                history_routed_experts = history_routed_experts[:history_routed_experts_expect_len]
                history_routed_experts_len = len(history_routed_experts)
            if cur_routed_experts_len != routed_experts_expect_len:
                self.logger.error(
                    "Current routed_experts len mismatch before partial rollout concatenation: "
                    f"cur_routed_experts_len={cur_routed_experts_len}, expected_len={routed_experts_expect_len}, "
                    f"prompt_tokens={prompt_tokens}, completion_tokens={completion_tokens}, "
                    f"history_routed_experts_len={history_routed_experts_len}, "
                    f"current_response_ids_len={len(response_ids)}"
                )
                cur_routed_experts = cur_routed_experts[:routed_experts_expect_len]
                cur_routed_experts_len = len(cur_routed_experts)
            if history_routed_experts_len - 1 > cur_routed_experts_len:
                self.logger.error(
                    "Current routed_experts len is shorter than history during partial rollout concatenation: "
                    f"history_routed_experts_len={history_routed_experts_len}, "
            ...
            if len(concat_routed_experts) != routed_experts_expect_len:
                self.logger.error(
                    "Concatenated routed_experts len mismatch after partial rollout concatenation: "
                ...
                concat_routed_experts = concat_routed_experts[:routed_experts_expect_len]
