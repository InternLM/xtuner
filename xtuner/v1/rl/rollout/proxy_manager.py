...
from xtuner.v1.rl.utils.misc import (
    check_chat_completions,
    delete_from_routedapiproxy,
    register_to_routedapiproxy,
)
...
class RolloutProxyManager(RolloutWorkerLifecycleListener):
    ...
    def __init__(self, config: "RolloutConfig"):
        ...
        self._routed_proxy_url = config.routed_proxy_url
        self._routed_proxy_admin_url = config.routed_proxy_admin_url
    ...
    def replace_registered_session_urls(self, session_urls: Iterable[str]) -> None:
        ...
        delete_from_routedapiproxy(self._routed_proxy_admin_url, model_name)
        ...
        for session_url in unique_session_urls:
            register_result = self._register_session_url(session_url, raise_on_error=True, log_success=False)
        self._logger.info(
            f"Registered {len(unique_session_urls)} rollout session servers to routed API proxy. "
            f"session_urls={unique_session_urls}, proxy_result={register_result}"
        )

    def validate_registered_session_urls(self) -> None:
        if not self._registered_session_urls:
            return
        self._validate_routed_proxy()
    ...
    def _register_session_url(
        ...
        *,
        ...
        model_name = self._get_model_name()
        try:
            result = register_to_routedapiproxy(self._routed_proxy_admin_url, model_name, session_url)
    ...
    def _delete_session_url(self, session_url: str | None, *, raise_on_error: bool = False) -> None:
        if session_url is None or session_url not in self._registered_session_urls:
            return
        try:
            delete_from_routedapiproxy(self._routed_proxy_admin_url, self._get_model_name(), session_url)
    ...
    def _validate_routed_proxy(self) -> None:
        model_name = self._get_model_name()
        self._logger.info(
            f"Validating routed API proxy chat completions: url={self._routed_proxy_url}, model={model_name}"
        )
        for attempt in range(1, self._CHECK_ATTEMPTS + 1):
            if check_chat_completions(self._routed_proxy_url, model_name):
                return
            if attempt < self._CHECK_ATTEMPTS:
                self._logger.warning(
                    f"check chat completions failed for routed API proxy {self._routed_proxy_url}, "
                    f"retrying {attempt}/{self._CHECK_ATTEMPTS - 1} after {self._CHECK_INTERVAL_SECONDS}s"
                )
                time.sleep(self._CHECK_INTERVAL_SECONDS)
        raise RuntimeError(f"check chat completions failed for routed API proxy: {self._routed_proxy_url}")
