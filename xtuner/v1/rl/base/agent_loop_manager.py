...
class AgentLoopManager:
    def __init__(
        self, agent_loop: AgentLoop, produce_strategy: ProduceStrategy, sampler: Sampler, replay_buffer: ReplayBuffer
    ):
        # 一一绑定
    ...
    # 共卡
    async def produce_batch(self, task_name: str, batch_size: int, prompt_k: int):
        await self._scheduler.produce_batch(self._agent_loop, self._data_sampler, batch_size, prompt_k, task_name)
        return await self._replay_buffer.get(batch_size, task_name, Status.COMPLETED)
