...
Splitting the session down here means the engine has to drive it across a package boundary, which
is the *only* reason the session's API is public:

- :func:`marker_session` opens a session for one pass through a checkpointed region, and must be
  entered inside the checkpointed callable so that the recompute pass rebuilds its own state.
- :func:`active_marker_session` is what the per-op policy asks whether a region is currently open.
- :class:`MarkerSession` carries that state: ``keeping`` (is any interval open), ``record`` (driven
  by :func:`checkpoint_record`), ``note_kept`` (the policy reporting that it kept an op) and
  ``finish`` (fold this pass into the owner's diagnostics).
- :func:`declare_selective_regions` tells the diagnostics how many layers to wait for before
  concluding that an interval kept nothing.

Treat that surface as internal to the engine rather than as an extension point: it is public
because of where the package boundary fell, and it carries no stability promise.
"""
