...
class MaybeCompile(Generic[P, T]):
    ...
    **Top(Module) level function**.
    ...
    def __init__(self, func: Callable[P, T]):
        ...
        # Resolve the qualname while `func` is still narrowed to `FunctionType`;
        # the cast below erases that narrowing for the rest of the class.
        self._origin_qualname = get_function_full_qualname(func)
    ...
    def enable_compile(self, **compile_options) -> None:
        ...
        if not is_compiled_function(self.func):
            ...
            self.func = wrap_with_profile_range(compiled, self._origin_qualname)
