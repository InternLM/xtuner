...
class Glm52MoE(MoE):
    ...
    def _decoder_stack(
        ...
        *,
        ...
        for idx, decoder_layer in self.layers.items():
            ...
            layer_inputs = (hidden_states,) if dsa_topk_ids is None else (hidden_states, dsa_topk_ids)
            offload_tensors = (
            ...
            assert isinstance(layer_results, tuple)
            if layer_idx < self.config.first_k_dense_replace:
                assert len(layer_results) == 2
                hidden_states, dsa_topk_ids = layer_results
            else:
                assert len(layer_results) == 5
                hidden_states, router_results, router_weights, router_topk_ids, dsa_topk_ids = layer_results
                if keep_router:
    ...
    def _micro_batch_decoder_stack(
        ...
        *,
        ...
        """Run GLM layers for micro-batches with flat checkpoint
        inputs/results."""
        activation_offload = int(os.getenv("XTUNER_ACTIVATION_OFFLOAD", "0")) == 1
        dsa_topk_offload = int(os.getenv("XTUNER_DSA_TOPK_OFFLOAD", "0")) == 1
        ...
        for idx, decoder_layer in self.layers.items():
            ...
            layer_inputs = list(hidden_states_list)
            if dsa_topk_ids_list is not None:
                layer_inputs.extend(dsa_topk_ids_list)
            ...
            assert isinstance(layer_results, tuple)
            if layer_idx < self.config.first_k_dense_replace:
                assert len(layer_results) == 2 * n
                hidden_states_list = list(layer_results[:n])
                dsa_topk_ids_list = list(layer_results[n:])
                continue

            assert len(layer_results) == 5 * n
            hidden_states = layer_results[:n]
            router_logits = layer_results[n : 2 * n]
            router_weights = layer_results[2 * n : 3 * n]
            router_topk_ids = layer_results[3 * n : 4 * n]
            dsa_topk_ids_list = list(layer_results[4 * n :])
