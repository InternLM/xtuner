...
class MoE(BaseModel):
    ...
    def _z_loss_dist_token_count(
        ...
        Returns the global non-padding token count, or ``None`` when there is no z-loss context or
        no process group is initialized (single-process reference / eval, handled as world size 1).
        """
        if z_ctx is None:
            return None
