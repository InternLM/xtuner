...
class MoE(BaseModel):
    ...
    def _micro_batch_forward(
        ...
        for idx, decoder_layer in self.layers.items():
            ...
            if layer_idx < self.config.first_k_dense_replace:
                ...
                for i, hidden_states in enumerate(layer_results["hidden_states"]):
    ...
    def default_recompute_cfg(self) -> RecomputeIntervalMap:
        ...
        - ``ep_size > 1``: only ``SAVE_MOE_DISPATCH`` survives, because the dispatcher calls it wraps are not
          compiled. ``SAVE_ATTN`` and ``SAVE_MOE_GATE`` are marked inside the compiled ``_pre_moe_forward``, and
          ``SAVE_MLP`` is marked in eager but encloses the compiled ``_shared_experts_forward``, so all three keep
          nothing.
