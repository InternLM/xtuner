...
class LayerInputMB(TypedDict):
    """Micro-batch counterpart of :class:`LayerInput`; every field is a per-microbatch list."""
...
class MoE(BaseModel):
    ...
    def get_layer_by_name(self, name: str) -> MoEDecoderLayer:
        """Resolve a routed-layer name to the decoder layer that owns its router.
    ...
    def update_bias(
        ...
        for layer_name, expected_load, current_loads in zip(
            layer_names, expected_loads, total_expert_counts_pre_iter
        ):
    ...
    def _prepare_hidden_states_mb(self, seq_ctx_list: list[SequenceContext]) -> LayerInputMB:
        """Embed every micro-batch into a per-MB hidden-state list (mb counterpart of
        :meth:`_prepare_hidden_states`).
    ...
    def _micro_batch_forward(
        ...
        output: dict = {}
        output |= self._mtp_outputs_mb(hidden_states_list, seq_ctx_list, loss_ctx_list, layer_input, router_logits_list)
    ...
    def _accumulate_layer_router_stats_mb(
        ...
        """Concatenate a layer's per-MB router stats into the global token set and accumulate aux loss.

        No-op for layers that don't contribute (e.g. hash-routed). The z-loss carrier is pinned to
        MB0's hidden_states: all MBs converge into the same total_loss backward, so MB0's path
        traverses every aux-loss node exactly once.
        """
    ...
    def _stack_router_logits_mb(
        ...
        """Stack the retained per-MB router logits into ``[1, n_mb, ...]`` tensors keyed by layer name."""
    ...
    def _run_dense_layers_mb(
        ...
        """Run the dense-prefix layers on the concatenated batch, returning a fresh per-MB list.
    ...
    def _mtp_outputs_mb(
        ...
        """Run the MTP block over every micro-batch and return ``{"mtp_loss": ...}`` (empty when off).
    ...
    def _prepare_hidden_states(self, seq_ctx: SequenceContext) -> LayerInput:
        """Embed inputs, build shared position embeddings, and mark dynamic shapes.
    ...
    def _activation_offload_ctx(
        ...
        """Activation-offload context that stages ``tensors`` (a layer's inputs) on CPU.
    ...
    def _decoder_layer_offload_ctx(self, decoder_layer, idx: str, layer_input: torch.Tensor):
        """Activation-offload context wrapping one single-sequence decoder-layer call.
    ...
    def _post_layer(self, hidden_states: torch.Tensor, idx: str, seq_ctx: SequenceContext) -> torch.Tensor:
        ...
        Runs after each decoder layer and its aux-loss accumulation. Subclasses that
        inject per-layer side inputs (e.g. multi-scale visual embeds) override this.
        """
        return hidden_states

    def _finalize_hidden_states(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """Transform the stacked hidden state before the final norm (default: identity).
    ...
    def _should_finalize_aux_loss(self) -> bool:
        """Whether to run :meth:`AuxLossContext.finalize` after the stack (default: True).

        Subclasses where no layer accumulates routing stats (e.g. an all-hash-routed
        sub-stack) override this to skip the finalize, which would otherwise raise.
        """
        return True
    ...
    def _forward(
        ...
        output |= self._mtp_outputs(
            layer_hidden_states, seq_ctx, loss_ctx, layer_input["position_embeddings"], aux_inputs, router_logits, router_weights
        )
        output |= self._finalize_aux_loss_outputs(aux_inputs)
    ...
    def _mtp_outputs(
        ...
        """Run the MTP block and return ``{"mtp_loss": ...}`` (empty dict when MTP is off).
