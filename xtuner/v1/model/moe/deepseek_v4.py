...
def _expand_hc_streams(hidden_states: torch.Tensor, hc_mult: int) -> torch.Tensor:
    """Expand ``[B, S, D]`` → ``[B, S, hc_mult, D]`` for the HC residual stream.
...
class V4LayerInput(LayerInput):
    """DeepSeek-V4's :class:`LayerInput`: adds the compressed rope and raw ``input_ids`` that every
    V4 layer consumes (HashRouter reads ``input_ids``; the Indexer reads the compressed rope)."""
...
class V4LayerInputMB(LayerInputMB):
    """Micro-batch counterpart of :class:`V4LayerInput`; every added field is a per-microbatch list."""
...
class DeepSeekV4(MoE):
    ...
    * **Forward override** — the standard :class:`MoE._forward` assumes
    ...
    @override
    def _call_decoder_layer_mb(self, decoder_layer, idx, hidden_states_list, seq_ctx_list, layer_input: V4LayerInputMB):  # type: ignore[override]
