...
class BaseModel(nn.Module):
    ...
    def _split_ignored_params(self, params: list[tuple[torch.Tensor, LoadSpec]]):
        ignored_params = [(p, l) for p, l in params if self._is_ignored_params(l.hf_keys[0])]
        params = [(p, l) for p, l in params if not self._is_ignored_params(l.hf_keys[0])]
        return ignored_params, params
