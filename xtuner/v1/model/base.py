...
class ModelOutputs(PydanticBaseModel):
    ...
    # The single total loss the engine backprops (LM/policy + aux + MTP, folded by the model).
    ...
    loss_log: dict[str, torch.Tensor] | None = None
...
class BaseModel(nn.Module):
    ...
    def _reduce_replicated_ignored_grads(self, params: Iterable[tuple[str, nn.Parameter]]) -> None:
        ...
        #
        # Invariant: SUM over the replicate group is correct only when each rank's grad is that rank's
        # OWN local contribution (partition), so summing partitions reconstructs the global grad.
        # TODO(reduce-sum): the replicate group includes the SP dim. Under a TPSP scheme where SP is
        # realized by an all-gather before attention, the attention block (including any QK norm) runs
        # over the full gathered sequence, so a replicated param's grad on each SP rank may be the
        # FULL-sequence grad rather than a per-rank partition. Whether this SUM then over-counts such a
        # grad by the SP size is an open point that has not been specifically handled here; revisit and
        # add a per-param SP-scaling guard if a TPSP path is confirmed to differentiate these params
    ...
    def post_micro_batch_forward(self, batch_outputs: Sequence[ModelOutputs]) -> BatchForwardInfo:
        ...
        # This rank's displayed total loss: sum of the per-rank loss terms. `*_global` keys are
        # cross-rank monitoring curves (e.g. balancing_loss_global), NOT additive loss terms, so they
        # are excluded to avoid double-counting the term they mirror.
        total_loss = sum(v for k, v in logs_info.items() if not k.endswith("_global"))
        for output in batch_outputs:
            if "extra_info" in output:
                train_engine_extra_info.append(output["extra_info"])
        return BatchForwardInfo(total_loss=total_loss, logs_info=logs_info, extra_info=train_engine_extra_info)
    ...
    def reduce_display_losses(self, batch_outputs: Sequence[ModelOutputs]) -> dict[str, float]:
        ...
        Naming note: "reduce" / the ``reduced_`` key prefix here means reduced ACROSS THE STEP'S
        MICRO-BATCHES on this rank, following the project-wide ``reduced_<name>`` logging convention.
        It is NOT a cross-rank reduction: each loss context computes its own detached per-rank display
        value via ``calibrate()`` (this rank's per-token / per-rank mean, no cross-rank all_reduce),
        stored on ``output.loss_log``, and here we only sum those over the micro-batches. So
        every rank logs ITS OWN loss (equal to the global loss at world size 1). The one cross-rank
        term is ``balancing_loss_global`` (all-reduced inside the loss context, identical on every
        rank). Subclasses may override.
        ...
        for output in batch_outputs:
            for name, value in (output.loss_log or {}).items():
