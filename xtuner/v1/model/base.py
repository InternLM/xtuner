...
class BaseModel(nn.Module):
    ...
    def __init__(self, config: XTunerBaseModelConfig):
        ...
        self._pending_async_hf: _AsyncHFSaveHandle | None = None
    ...
    def async_save_hf(
        ...
        for filename, names in file_to_names:
            ...
            save_futures.append(executor.submit(_save_file, tensors, tmp_hf_dir / filename))

        self._pending_async_hf = _AsyncHFSaveHandle(
            futures=save_futures,
            executor=executor,
            hf_dir=hf_dir,
    ...
    def wait_async_hf(self) -> Path | None:
        ...
        try:
            for future in handle.futures:
                future.result()
            local_weight_map = handle.weight_map
        ...
        if rank == 0:
            self._write_hf_index_and_config(hf_dir=handle.tmp_hf_dir, weight_map=merged_weight_map)
        if dist.is_initialized():
            dist.barrier()
        if rank == 0:
            if handle.hf_dir.exists():
                rmtree(handle.hf_dir)
            handle.tmp_hf_dir.rename(handle.hf_dir)
        if dist.is_initialized():
            dist.barrier()
        self._pending_async_hf = None
        return handle.hf_dir
