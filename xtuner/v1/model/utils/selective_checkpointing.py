...
def apply_selective_checkpointing(
    ...
    *,
    ...
    if uses_dsa_topk_lifecycle(module):
        ...
        if kept_intervals:
            _warn_intervals_unsupported(module, kept_intervals, "it uses DSA cross-layer top-k sharing")
...
def _warn_intervals_unsupported(module: nn.Module, intervals: tuple[MarkerInterval, ...], reason: str) -> None:
    # Reported here rather than left to `MarkerSession.report_unreached`, which never speaks for
