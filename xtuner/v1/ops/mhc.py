...
def is_available() -> bool:
    """``True`` if ``tile-kernels`` is importable. Cheap to call repeatedly."""
    try:
...
def _head_compute_mix_bwd_op(
    ...
    kernel(flat_grad, flat_input, scale.contiguous().view(1), base.contiguous(),
           input_grad, scale_grad_partial, base_grad_partial)
...
def mhc_post(x: Tensor, residual: Tensor, post: Tensor, comb: Tensor) -> Tensor:
    """V4 ``hc_post``: ``out[..., h, d] = post[..., h] * x[..., d] +
    sum_{h'} comb[..., h', h] * residual[..., h', d]``.
...
def mhc_expand(hidden_states: Tensor, hc_mult: int) -> Tensor:
    """Broadcast ``[..., hidden] → [..., hc_mult, hidden]`` for HC residual streams.
...
def _pre_split_mixes_bwd_op(
    pre_grad: Tensor, post_grad: Tensor, comb_grad: Tensor,
    mixes: Tensor, post_out: Tensor, scale: Tensor, base: Tensor, post_mult_value: float,
) -> tuple[Tensor, Tensor, Tensor]:
    *batch, mhc_mult3 = mixes.shape
    n = 1
    for b in batch:
    ...
    kernel(
        ...
        mixes_flat, post_flat, scale.contiguous().view(3), base.contiguous(),
        mixes_grad, scale_grad_partial, base_grad_partial,
    )
    return (
...
def _pre_split_mixes_backward(ctx, pre_grad, post_grad, comb_grad):
    ...
    gm, gs, gb = _pre_split_mixes_bwd_op(
        pre_grad.contiguous(), post_grad.contiguous(), comb_grad.contiguous(),
        mixes, post_out, scale, base, ctx.post_mult_value,
    )
...
def mhc_sinkhorn(x: Tensor, repeat: int, eps: float) -> Tensor:
    """Doubly-stochastic projection of a per-token ``[hc_mult, hc_mult]`` matrix.
