...
def _get_petrel_client():
    """Lazy initialization of petrel client for OSS/Ceph storage.
    
    Uses PETREL_CONF_PATH environment variable if set.
    """
    global _petrel_client
    if '_petrel_client' not in globals() or _petrel_client is None:
        try:
            import os
            from petrel_client.client import Client
            conf_path = os.environ.get("PETREL_CONF_PATH")
            if conf_path:
                _petrel_client = Client(conf_path=conf_path)
                logger.info(f"Initialized petrel client with config: {conf_path}")
            else:
                _petrel_client = Client()
                logger.info("Initialized petrel client with default config")
        except ImportError:
            logger.warning("petrel_client not available, hceph: paths will not work")
            _petrel_client = None
    ...
    if isinstance(image, Image.Image):
        ...
        if "base64," in image:
            ...
            with BytesIO(data) as bio:
                image_obj = copy.deepcopy(Image.open(bio))
    elif image.startswith("hceph:") or image.startswith("s3://"):
        # Handle OSS/Ceph storage paths
        client = _get_petrel_client()
        if client is None:
            raise ValueError(f"petrel_client not available, cannot load image from {image}")
        # Remove hceph: prefix if present
        # oss_path = image[6:] if image.startswith("hceph:") else image
        oss_path = image
        try:
            img_bytes = client.get(oss_path)
            with BytesIO(img_bytes) as bio:
                image_obj = copy.deepcopy(Image.open(bio))
        except Exception as e:
            raise ValueError(f"Failed to load image from OSS: {image}, error: {e}")
    else:
