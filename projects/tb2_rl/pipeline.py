"""tb2-rl pipeline factory — mirrors claw_bench.pipeline.
...
def tb2_rl_pipeline(
    *,
    ...
    """Build a Runner for a tb2-rl task.

    Args:
        sandbox (SandboxSpec): Sandbox image + workspace config.  Defaults to
            ``t-data-processing-v1`` at ``/app``.
    ...
    infer = SandboxStage(
        ...
        pre=[
            ...
            # 5. Upload environment/files/* flat into the workspace (what the
            #    Dockerfile would have COPYd to /app/ at image-build time).
            UploadHook(
                [
                    {
