"""tb2-eval rollout runner config.
...
``task.toml [environment] docker_image``.  The image is passed at runtime in
the pipeline config's ``sandboxes.main`` entry (see ``tb2_eval_tokenize_fn.py``).

The ``DEFAULT_SANDBOX`` below uses a placeholder image; in practice the
per-task image always overrides it via the sample's pipeline config.
"""
...
# Placeholder image — each task overrides this via its pipeline config.
    ...
    infer=dict(
        ...
        pre=[
            dict(
                type=UploadHook,
                mappings=[
                    dict(base=str(WRAPPERS / "tb2_eval"), source="*", target=PATHS.wrappers_bench + "/", flatten=True),
                ],
            ),
            ...
            dict(
                ...
                stage=dict(
                    ...
                    pre=[
                        dict(
                            type=UploadHook,
                            mappings=[dict(base="tests", source="**/*", target=f"{PATHS.tests}/")],
                        ),
                    ...
                    entries=[
                        dict(
                            type=ShellEntry,
                            name="run_tests",
                            cmd=f"bash {PATHS.wrappers_bench}/run_tests.sh",
                                        timeout=900,
                        )
                    ],
                    env={
