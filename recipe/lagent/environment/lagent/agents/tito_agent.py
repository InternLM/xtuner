...
class AsyncTokenInOutAgentMixin:
    async def __call__(self, *message: AgentMessage, session_id=0, **kwargs) -> AgentMessage:  # type: ignore[override]
        ...
        _message = self._scroll_buffer(message[-1], session_id)  # type: ignore[attr-defined]
        if _message is not None:
            ...
            self.update_memory(message, session_id=session_id)  # type: ignore[attr-defined]
        ...
        self.update_memory(response_message, session_id=session_id)  # type: ignore[attr-defined]
    ...
    async def forward(self, *message: AgentMessage, session_id=0, **kwargs) -> AgentMessage:
        ...
        if partial_response and partial_response.raw_content:
            self.update_memory(partial_response, session_id=session_id)  # type: ignore[attr-defined]
        formatted_messages = self.aggregator.aggregate(  # type: ignore[attr-defined]
            self.memory.get(session_id),  # type: ignore[attr-defined]
            self.name,  # type: ignore[attr-defined]
            self.output_format,  # type: ignore[attr-defined]
            self.template,  # type: ignore[attr-defined]
        )
        response_message = await self.llm.chat(formatted_messages, session_id, **kwargs)  # type: ignore[attr-defined]
        ...
        if partial_response and partial_response.raw_content:
            ...
            self.memory.get(session_id).delete(-1)  # type: ignore[attr-defined]
        return response_message
