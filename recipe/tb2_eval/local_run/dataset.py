"""tb2-eval dataset: read prebuilt JSONL records, yield AgentRolloutItem.

The JSONL is produced by ``recipe.tb2_eval.scripts.generate_jsonl``. Each
record fully describes a task (id, instruction, tags, ability,
pipeline_overrides), so this module does nothing but field-mapping — no
``task.toml`` parsing.
"""
...
class TB2EvalBench:
    """tb2-eval dataset iterator.  One pipeline (with per-task overrides) for
