...
- `analyzer/cli.py`: 离线分析命令入口。加载 `expected.yaml`，构造 `AnalyzerContext`，解析 `checker: module.path:function` 字段，通过 `importlib.import_module(module_path)` 和 `getattr(module, function_name)` 找到 checker 并执行。
- `analyzer/context.py`: 定义 `AnalyzerContext`，统一承载 work_dir、expected、从正常日志中提取出的 case 相关信息、tracker rows、train rollout 数据和原始 log 路径。
...
如果为了少改 `RLColocateTrainer._sync_weights_and_save()`，也可以让 fake trainer 的 `update_weights()` 在非更新 step 直接 no-op 并打印普通日志说明 no-op；checker 负责确认只有 expected update step 出现 version 增长。
...
checker 验证 `tail_expired` 时只检查 step-based staleness：
...
每个 case 的 checker 可以按自己的需求解析正常日志。允许不同 case 使用不同提取逻辑，但关键 fake 事件第一版就使用 JSON 片段，避免字段顺序、空格或措辞变化导致 checker 脆弱。

如果某个 checker 无法从现有日志判断正确性，再在最接近事实发生的位置补充一条普通 logger 日志。新增日志应满足：

- 使用现有 logger 正常打印，不引入特殊前缀协议。
- 从第一版开始，关键 fake 日志使用“普通说明 + JSON 片段”的形式，checker 使用 `json.loads()` 解析 JSON 片段，不依赖字段顺序、空格或自然语言。
...
- fake rollout、fake judge 可以在 actor 日志中打印，checker 必须按 `(event, step, task, uid, attempt)` 或 case 自己定义的 key 聚合。
- fake trainer 的 `train_fit_done` 和 `weight_update_done` 默认只由 `FakeTrainingController` 打印一次。
- 如果同一事件被重复打印，checker 必须显式去重，不能依赖文件读取顺序。
- 单机 fake 运行时可能只有 `rank_0.log`，也可能因为 Ray actor logger 行为产生多个 log；loader 不假设固定文件数量。
- loader 不按 wall-clock 合并日志；checker 用 step/task/uid/attempt/version 等业务字段判断。
...
- 如果某个 case 需要判断“哪个 uid 在第几步 completed / aborted”，checker 从 producer/agent loop 日志和 train rollout 产物中提取 step；如果信息不足，再补普通 logger 日志。
- 默认 checker 应比较集合或 multiset，而不是比较原始日志行顺序。
...
```python
def check(ctx: AnalyzerContext) -> CheckResult:
    ...
```
...
- `expected.yaml`: 定义这个 case 要验证什么。不写执行逻辑，只写期望和阈值。
- `checker.py`: 定义这个 case 怎么从日志和产物里提取信息，并判断是否符合 `expected.yaml`。

公共 analyzer 不理解 case 的业务含义，只做三件事：

1. 加载日志和产物，构造 `AnalyzerContext`。
2. 根据 `expected.yaml` 中的 `checker` 字段 import 对应 `checker.py`。
3. 调用 `check(ctx)`，写出 summary/failures。

这样后续新增 case 时，不需要改公共 analyzer；只有当公共 loader 缺少某种通用产物读取能力时，才扩展 analyzer。

## Checker 编写约定

每个 checker 都应该保持结构清晰，建议按四段写：
...
def check(ctx: AnalyzerContext) -> CheckResult:
    ...
    failures.extend(check_xxx(extracted, expected))
    failures.extend(check_yyy(extracted, expected))
...
推荐每个 checker 至少拆出这些函数：

- `extract(ctx)`: 只负责从 `ctx.log_records`、`ctx.tracker_rows`、`ctx.train_rollouts` 中提取当前 case 关心的信息，不做判断。
- `check_*(extracted, expected)`: 每个函数只检查一类规则，例如 weight sync、partial rollout、fault filtering、task allocation。
...
`expected.yaml` 只描述“期望”，不写 Python 逻辑。

通用字段：

```yaml
case_name: async_update_every_4
checker: recipe.rl_simulator.cases.async_update_every_4.checker:check
seed: 20260421
...
case-specific 字段由各自 checker 解释，例如：

```yaml
weight_sync:
  expected_update_steps: [4, 8]
  forbidden_update_steps: [1, 2, 3, 5, 6, 7]
  expected_rollout_weight_version_by_step:
    1: 0
    2: 0
    3: 0
    4: 0
...
```yaml
partial_rollout:
  require_any_aborted_then_completed: true
  max_tokens: 512
  max_seq_staleness: 2
```
...
- 公共字段越少越好。
- 新 case 可以自由增加自己的 expected 字段。
- 如果某个字段会被多个 case 复用，再考虑沉淀成公共 checker helper。
...
3. 修改 `expected.yaml`，写清楚这个 case 想证明什么。
4. 写 `checker.py` 的 `extract()`，先生成 summary，不急着加很多 check。
5. 看 summary 是否已经包含判断所需信息。
6. 如果信息不够，再在 fake 组件或少量核心路径补普通 logger 日志。
7. 补 `check_*()`，让 analyzer 能 pass/fail。
8. 故意改错一个 expected，确认 checker 能 fail，并且 failure 信息能定位问题。
...
- 每个 case 的 `expected.yaml` 必须写默认 seed。
- runner 默认使用 case config / expected 中的 seed，不做随机覆盖。
- 如果命令行传了 `--seed`，最终 seed 必须写入 `simulation_run_manifest.json`，analyzer summary 也要显示这个 seed。
- 单 case debug 时，推荐固定 `--work-dir`；失败后可以用同一条命令复现，也可以用 `--analyze-only` 反复调 checker。
...
- checker 从 manifest 读取两次运行目录，不从 `expected.yaml` 写死 `compare_work_dir`。
- checker 比较两次 normalized digest，忽略 timestamp、pid、hostname、log 文件名和日志行顺序。
