...
class RLTB2RLTokenizeFn(CachableTokenizeFunction[RolloutState]):
    """tb2-rl tokenize function aligned with ``RLTextTokenizeFn``."""

    DATA_SOURCE_NAME = "tb2-rl"
    PIPELINE_DOTTED = "recipe.tb2_rl.pipeline.runner"
...
class RLTB2RLTokenizeFnConfig(BaseModel):
    model_config = ConfigDict(title="tb2-rl RL dataset config for xtuner", extra="forbid")
    max_length: int | None = None
    tools_schema: list | None = None
    data_judger_mapping: dict | None = None
    system_prompt: str | None = None

    def build(self, tokenizer: PreTrainedTokenizer, **kwargs) -> RLTB2RLTokenizeFn:
