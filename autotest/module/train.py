...
class Train:
    def get_cmd(config):
        ...
        if train_type in ["sft", "rl"]:
            ...
            if train_type == "sft":
                command = (
                    f"cd {current_dir}; pwd; pip install -e .[all]; pip install more-itertools; pip show transformers; pip show torch; export GITHUB_RUN_ID={config.get('run_id')}; export WORK_DIR={work_dir}; "
