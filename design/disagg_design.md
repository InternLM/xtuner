...
- `cleanup_pending_tasks(pause_product_for_update: bool = False) -> float`
...
- 这些 pending 任务需要跨调用保留，并在合适的时候统一 cleanup
...
### 5.3 `cleanup_pending_tasks()` 外提

旧逻辑里，strategy 会在 `produce_batch()` 末尾自动 cleanup pending rollout。

新逻辑把 cleanup 提升到 manager / trainer 外层，原因是：

- 非共卡下，什么时候停 producer，不应由 strategy 自己决定
- trainer 需要在权重同步前明确地说“现在停下来”
- cleanup 的耗时也应作为一次显式操作被记录

因此：

- `AsyncProduceStrategy.cleanup_pending_tasks()` 负责真正停止并回收 pending rollout
- `AgentLoopManager.cleanup_pending_tasks()` 负责在单 task / 多 task 情况下统一调度各 strategy 的 cleanup
...
- `cleanup_pending_tasks()` 返回 `pause_time_s`
...
4. 把最近一次 cleanup 的 `pause_time_s` 带给结果

### 7.3 `cleanup_pending_tasks(...)`

当 `pause_product_for_update=True` 时：

1. 先置 `_update_event`
2. 再把 manager 状态切到 `UPDATE_ABORT`
3. 最后调用各 task strategy 的 `cleanup_pending_tasks()`

顺序很重要。

先置事件的原因是：

- 防止 producer 在 cleanup 开始前又继续补发新任务
...
3. `cleanup_pending_tasks(pause_product_for_update=False)`
...
- `UPDATE_ABORT`
  ...
  - producer 不再自己 cleanup，避免与 trainer 竞争
...
- producer loop 收到 `UPDATE_ABORT` 后不做二次 cleanup
- 只有 trainer 的权重同步路径显式调用 `cleanup_pending_tasks()`
...
1. `agent_loop_manager.cleanup_pending_tasks(pause_product_for_update=True)`
...
1. `agent_loop_manager.cleanup_pending_tasks(pause_product_for_update=True)` 已完成
...
- save 的时机在 `cleanup_pending_tasks(pause_product_for_update=True)` 之后
...
- checkpoint 是在 cleanup 后、continue_product 前拍摄的
...
- `cleanup_pending_tasks(pause_product_for_update=True)` 先置 `_update_event`
...
- checkpoint 保存点发生在 cleanup 之后、continue_product 之前
...
- cleanup 只由 trainer 的权重同步路径显式触发一次
  - producer 收到 `UPDATE_ABORT` 后不再自行二次 cleanup

- rollout 当前只支持 `abort_all`
  - 不做按 request 的精细化取消
  - 活跃 pending 统一在 cleanup 阶段处理
