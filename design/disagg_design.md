...
- `cleanup_pending_tasks(for_weight_update: bool = False) -> float`
- `reset(model_rollout_step: int) -> None`
...
`reset(model_rollout_step=...)` 的作用不是清空 replay buffer，而是恢复 producer 控制状态：
...
3. `cleanup_pending_tasks(for_weight_update=False)`
...
4. 如果这一轮需要 eval
   - 先做 eval
5. `agent_loop_manager.reset(model_rollout_step=current_step)`
...
### 12.2 为什么 eval 要优先于 producer reset

如果同步权重后立刻 `reset()`，producer 会马上恢复后台生成。
