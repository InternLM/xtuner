...
torchrun --master-port 12384 --nproc-per-node 16 --nnodes=$WORLD_SIZE --master-addr $MASTER_ADDR --master-port $MASTER_PORT --node-rank $RANK -m xtuner.v1.train.cli.sft --config ci/config/qwen3_235B_vl.py
